  ...
  return (
    <DashboardShell activeId="risk">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '18px', borderBottom: '1px solid ' + border }}>
        <div>
          <h1 style={{ fontSize: '26px', fontWeight: '900', letterSpacing: '-0.02em', margin: '0 0 6px', color: '#ffffff' }}>Risk Intelligence™</h1>
          <div style={{ fontSize: '12px', color: '#666' }}>Identify, assess and monitor the risks that could impact your business performance.</div>
        </div>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            <div style={{ display: 'flex', flexDirection: 'column' as const, justifyContent: 'space-between', paddingBottom: '18px' }}>
              {['Very High','High','Medium','Low','Very Low'].map(l => (
                <div key={l} style={{ fontSize: '7.5px', color: '#444', textAlign: 'right' as const, width: '38px', lineHeight: '26px' }}>{l}</div>
              ))}
            </div>
            <div style={{ flex: 1 }}>
                ...
                {[5,4,3,2,1].map(row => [1,2,3,4,5].map(col => {
                  ...
                  return (
                    <div key={col} style={{ height: '26px', backgroundColor: heatColors[key] || '#0a0a0a', borderRadius: '3px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255,255,255,0.03)', gap: '1px' }}>
                      {ids.map(id => (
                        <div key={id} onClick={() => setSelectedRisk(risks[id-1])} style={{ width: '15px', height: '15px', borderRadius: '50%', backgroundColor: risks[id-1]?.sevColor || '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '7px', color: '#fff', fontWeight: '700', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.2)' }}>{id}</div>
                      ))}
                    </div>
                  )
                }))}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '3px', marginBottom: '2px' }}>
                {['Very Low','Low','Medium','High','Very High'].map(l => (
                  <div key={l} style={{ fontSize: '7px', color: '#444', textAlign: 'center' as const }}>{l}</div>
                ))}
              </div>
              <div style={{ fontSize: '8px', color: '#333', textAlign: 'center' as const, letterSpacing: '0.1em' }}>LIKELIHOOD</div>
            </div>
          </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '5px' }}>
            {risks.slice(0,5).map((r, i) => (
              <div key={i} onClick={() => setSelectedRisk(r)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', backgroundColor: '#0a0a0a', borderRadius: '7px', border: '1px solid ' + border, cursor: 'pointer' }}>
                ...
                <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                  <div style={{ fontSize: '11px', color: '#e0e0e0', fontWeight: '700' }}>{fmt(r.value)}</div>
                  <div style={{ fontSize: '8px', color: '#555' }}>Potential impact</div>
                </div>
            ...
            <div style={{ flex: 1 }}>
              {riskCategories.map((cat, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '4px' }}>
                  ...
                  <div style={{ flex: 1, fontSize: '10px', color: '#888' }}>{cat.label}</div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '6px', flexWrap: 'wrap' as const }}>
            {riskCategories.map((cat,i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                ...
                <span style={{ fontSize: '8px', color: '#555' }}>{cat.label.split(' ')[0]}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '680px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ fontSize: '22px', fontWeight: '800', color: '#fff', marginBottom: '4px' }}>{selectedRisk.name}</div>
                <div style={{ fontSize: '12px', color: '#666' }}>{selectedRisk.desc} · {selectedRisk.category}</div>
              </div>
              <button onClick={() => setSelectedRisk(null)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
              ...
              <div style={{ padding: '14px', backgroundColor: '#0a0a0a', borderRadius: '8px', border: '1px solid #1e1e1e' }}>
                ...
                {['Owner identified as key risk driver', 'Concentration above industry threshold', 'Verification score confirms exposure'].map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#aaa', lineHeight: '1.5' }}>{e}</span>
                  </div>
                ))}
              </div>
              <div style={{ padding: '14px', backgroundColor: '#0a0a0a', borderRadius: '8px', border: '1px solid #1e1e1e' }}>
            ...
            <div style={{ padding: '12px 14px', backgroundColor: 'rgba(204,68,68,0.06)', border: '1px solid rgba(204,68,68,0.2)', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '13px', color: '#ffffff', fontWeight: '600' }}>Deploy mitigation framework — estimated risk reduction: 40–60%</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showHeatMapModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowHeatMapModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '780px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK HEAT MAP</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Full Risk Heat Map Analysis</div></div>
              <button onClick={() => setShowHeatMapModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '16px' }}>
              {[{l:'CRITICAL ZONE',v:criticalCount+' risks',c:'#cc4444'},{l:'HIGH ZONE',v:highCount+' risks',c:'#e8923a'},{l:'MEDIUM ZONE',v:mediumCount+' risks',c:gold},{l:'LOW ZONE',v:lowCount+' risks',c:'#4aaa4a'}].map((m,i) => (
                <div key={i} style={{ padding: '12px', backgroundColor: '#0a0a0a', borderRadius: '8px', border: '1px solid '+m.c+'33', textAlign: 'center' as const }}>
            ...
            {risks.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', padding: '10px', backgroundColor: '#0a0a0a', borderRadius: '7px', border: '1px solid #1e1e1e', marginBottom: '6px', alignItems: 'center' }}>
      ...
      {showRiskRegisterModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowRiskRegisterModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '820px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK REGISTER</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Full Risk Register</div><div style={{ fontSize: '12px', color: '#666' }}>{businessName} · {risks.length} risks · Total exposure: {fmt(totalRiskValue)}</div></div>
              <button onClick={() => setShowRiskRegisterModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 0.8fr 0.8fr 0.8fr 1fr', gap: '0', padding: '8px 0', borderBottom: '1px solid #1a1a1a', marginBottom: '4px' }}>
              {['RISK','CATEGORY','SEVERITY','LIKELIHOOD','IMPACT','VALUE'].map(h => <div key={h} style={{ fontSize: '9px', color: '#444', letterSpacing: '0.08em', fontWeight: '600' }}>{h}</div>)}
            </div>
            {risks.map((r, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 0.8fr 0.8fr 0.8fr 1fr', gap: '0', padding: '10px 0', borderBottom: i < risks.length-1 ? '1px solid #111' : 'none', alignItems: 'center' }}>
                ...
                <div style={{ fontSize: '11px', color: '#888' }}>{r.impact}</div>
                <div style={{ fontSize: '12px', fontWeight: '700', color: r.sevColor }}>{fmt(r.value)}</div>
              </div>
            ))}
          </div>
      ...
      {showBreakdownModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowBreakdownModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK SCORE BREAKDOWN</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Risk Category Analysis</div></div>
              <button onClick={() => setShowBreakdownModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {riskCategories.map((cat, i) => (
              <div key={i} style={{ padding: '14px', backgroundColor: '#0a0a0a', borderRadius: '8px', border: '1px solid #1e1e1e', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '2px', backgroundColor: cat.color }} />
                    <span style={{ fontSize: '14px', color: '#e0e0e0', fontWeight: '700' }}>{cat.label}</span>
                  </div>
      ...
      {showTrendModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowTrendModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '720px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>TREND ANALYSIS</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Risk Score Trends — Last 90 Days</div></div>
              <button onClick={() => setShowTrendModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
      ...
      {showInsightsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowInsightsModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK CATEGORY INSIGHTS</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Full Category Intelligence</div></div>
      ...
      {showMitigationModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowMitigationModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '760px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>MITIGATION PLANS</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>All Mitigation Actions</div></div>
              <button onClick={() => setShowMitigationModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#888', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {[...mitigations,
      ...
      {showReportModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowReportModal(false)}>
          <div style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '720px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK INTELLIGENCE REPORT</div><div style={{ fontSize: '22px', fontWeight: '800', color: '#fff' }}>Full Risk Intelligence Report</div><div style={{ fontSize: '12px', color: '#666' }}>{businessName} · Score: {overallRiskScore}/100 · {risks.length} risks</div></div>
            ...
            <div style={{ padding: '14px', backgroundColor: '#0a0a0a', borderRadius: '8px', border: '1px solid #1e1e1e', marginBottom: '12px' }}>
            ...
            {risks.slice(0,5).map((r, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 4 ? '1px solid #111' : 'none' }}>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  {sevBadge(r.severity, r.sevColor)}
                  <span style={{ fontSize: '13px', color: '#e0e0e0', fontWeight: '600' }}>{r.name}</span>
                </div>
