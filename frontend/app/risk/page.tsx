...
export default function RiskIntelligencePage() {
  ...
  const sevBadge = (sev: string, color: string) => (
    <div style={{ padding: '2px 7px', backgroundColor: color + '18', border: '1px solid ' + color + '44', borderRadius: '4px', fontSize: '9px', color, fontWeight: '700', whiteSpace: 'nowrap' as const }}>{sev.toUpperCase()}</div>
  )
    ...
    <DashboardShell activeId="risk">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '18px', borderBottom: '1px solid ' + border }}>
        <div>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Identify, assess and monitor the risks that could impact your business performance.</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '10px' }}>Impact vs Likelihood</div>
          <div style={{ display: 'flex', gap: '4px' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '8px', color: 'var(--text-faint)', textAlign: 'center' as const, letterSpacing: '0.1em' }}>LIKELIHOOD</div>
            </div>
          </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>TOP RISK EXPOSURES</div>
            <button onClick={() => setShowRiskRegisterModal(true)} style={{ fontSize: '10px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View all →</button>
          </div>
            ...
            {risks.slice(0,5).map((r, i) => (
              <div key={i} onClick={() => setSelectedRisk(r)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '7px', border: '1px solid ' + border, cursor: 'pointer' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{r.name}</div>
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{r.desc}</div>
                </div>
                {sevBadge(r.severity, r.sevColor)}
                <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                  <div style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: '700' }}>{fmt(r.value)}</div>
                  <div style={{ fontSize: '8px', color: 'var(--text-muted)' }}>Potential impact</div>
                </div>
                <div style={{ fontSize: '14px', color: 'var(--text-secondary)' }}>›</div>
              </div>
            ))}
          </div>
          <button onClick={() => setShowRiskRegisterModal(true)} style={{ width: '100%', marginTop: '10px', padding: '8px', backgroundColor: 'transparent', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '6px', color: gold, fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>View full risk register →</button>
        </div>

        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>RISK SCORE BREAKDOWN</div>
            ...
            <div style={{ flex: 1 }}>
              {riskCategories.map((cat, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '4px' }}>
                  ...
                  <div style={{ flex: 1, fontSize: '10px', color: 'var(--text-muted)' }}>{cat.label}</div>
                  <div style={{ fontSize: '10px', color: cat.color, fontWeight: '700' }}>{cat.score}/100</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>RISK TRENDS OVER TIME</div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '6px', flexWrap: 'wrap' as const }}>
            {riskCategories.map((cat,i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                ...
                <span style={{ fontSize: '8px', color: 'var(--text-muted)' }}>{cat.label.split(' ')[0]}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>RISK CATEGORY INSIGHTS</div>
            <button onClick={() => setShowInsightsModal(true)} style={{ fontSize: '10px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View insights →</button>
          </div>
          {riskCategories.map((cat, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 0', borderBottom: i < riskCategories.length-1 ? '1px solid #111' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: '600' }}>{cat.label}</div>
              ...
              <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                <div style={{ fontSize: '10px', color: cat.trend > 0 ? '#cc4444' : '#4aaa4a', fontWeight: '600' }}>{cat.trend > 0 ? '↑ ' : '↓ '}{Math.abs(cat.trend)}</div>
                <div style={{ fontSize: '9px', color: cat.dir === 'Increasing' ? '#cc4444' : '#4aaa4a' }}>{cat.dir}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>RISK MITIGATION ACTIONS</div>
            <button onClick={() => setShowMitigationModal(true)} style={{ fontSize: '10px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View all actions →</button>
          </div>
          {mitigations.map((m, i) => (
            <div key={i} style={{ display: 'flex', gap: '8px', padding: '8px 0', borderBottom: i < mitigations.length-1 ? '1px solid #111' : 'none', alignItems: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{m.title}</div>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>{m.desc}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  {progressBar(m.progress, m.sevColor)}
                  <span style={{ fontSize: '11px', color: 'var(--text-muted)', flexShrink: 0 }}>{m.progress}%</span>
                </div>
              </div>
              {sevBadge(m.severity, m.sevColor)}
              <div style={{ fontSize: '10px', color: m.status === 'In Progress' ? '#4aaa4a' : '#555', fontWeight: '600', flexShrink: 0 }}>{m.status}</div>
            </div>
          ))}
          <button onClick={() => setShowMitigationModal(true)} style={{ width: '100%', marginTop: '10px', padding: '7px', backgroundColor: 'transparent', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '6px', color: gold, fontSize: '10px', fontWeight: '600', cursor: 'pointer' }}>View all mitigation plans →</button>
        </div>
      </div>

      <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
        <div style={{ fontSize: '10px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600', marginBottom: '12px' }}>RISK INTELLIGENCE SUMMARY</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: '10px', alignItems: 'stretch' }}>
          {[
            ...
            <div key={i} style={{ padding: '12px 14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid ' + border }}>
              <div style={{ fontSize: '9px', color: s.badgeColor, letterSpacing: '0.08em', fontWeight: '700', marginBottom: '7px' }}>{s.badge}</div>
              <div style={{ display: 'flex', gap: '7px', alignItems: 'center', marginBottom: '5px' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-primary)', fontWeight: '700', lineHeight: 1.2 }}>{s.name}</div>
              </div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{s.meta}</div>
            </div>
          ))}
        ...
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setSelectedRisk(null)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '680px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK DETAIL</div>
                <div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>{selectedRisk.name}</div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{selectedRisk.desc} · {selectedRisk.category}</div>
              </div>
              <button onClick={() => setSelectedRisk(null)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '10px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>RISK DRIVERS</div>
                {['Owner identified as key risk driver', 'Concentration above industry threshold', 'Verification score confirms exposure'].map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                    <span style={{ color: '#cc4444', fontSize: '10px', flexShrink: 0 }}>⚠</span>
                    <span style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.5' }}>{e}</span>
                  </div>
                ))}
              </div>
              <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '10px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>MITIGATION APPROACH</div>
                {['Immediate: implement risk controls', 'Short-term: develop contingency plan', 'Long-term: structural risk reduction'].map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                    <span style={{ color: '#4aaa4a', fontSize: '10px', flexShrink: 0 }}>→</span>
                    <span style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.5' }}>{e}</span>
                  </div>
                ))}
              </div>
            ...
            <div style={{ padding: '12px 14px', backgroundColor: 'rgba(204,68,68,0.06)', border: '1px solid rgba(204,68,68,0.2)', borderRadius: '8px' }}>
              <div style={{ fontSize: '10px', color: '#cc4444', letterSpacing: '0.12em', marginBottom: '6px', fontWeight: '600' }}>RECOMMENDED ACTION</div>
              <div style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: '600' }}>Deploy mitigation framework — estimated risk reduction: 40–60%</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showHeatMapModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowHeatMapModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '780px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK HEAT MAP</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Full Risk Heat Map Analysis</div></div>
              <button onClick={() => setShowHeatMapModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '16px' }}>
              {[{l:'CRITICAL ZONE',v:criticalCount+' risks',c:'#cc4444'},{l:'HIGH ZONE',v:highCount+' risks',c:'#e8923a'},{l:'MEDIUM ZONE',v:mediumCount+' risks',c:gold},{l:'LOW ZONE',v:lowCount+' risks',c:'#4aaa4a'}].map((m,i) => (
                <div key={i} style={{ padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid '+m.c+'33', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '9px', color: m.c, letterSpacing: '0.1em', marginBottom: '4px', fontWeight: '600' }}>{m.l}</div>
            ...
            {risks.map((r, i) => (
              <div key={i} style={{ display: 'flex', gap: '12px', padding: '10px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '7px', border: '1px solid var(--border)', marginBottom: '6px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: '700' }}>{r.name}</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{r.desc} · {r.category}</div>
                </div>
                <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Likelihood: <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>{r.likelihood}</span></div>
                <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Impact: <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>{r.impact}</span></div>
                {sevBadge(r.severity, r.sevColor)}
                <div style={{ fontSize: '12px', fontWeight: '700', color: r.sevColor }}>{fmt(r.value)}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {showRiskRegisterModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowRiskRegisterModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '820px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK REGISTER</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Full Risk Register</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{businessName} · {risks.length} risks · Total exposure: {fmt(totalRiskValue)}</div></div>
              <button onClick={() => setShowRiskRegisterModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 0.8fr 0.8fr 0.8fr 1fr', gap: '0', padding: '8px 0', borderBottom: '1px solid #1a1a1a', marginBottom: '4px' }}>
              {['RISK','CATEGORY','SEVERITY','LIKELIHOOD','IMPACT','VALUE'].map(h => <div key={h} style={{ fontSize: '9px', color: 'var(--text-secondary)', letterSpacing: '0.08em', fontWeight: '600' }}>{h}</div>)}
            </div>
            {risks.map((r, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '2.5fr 1fr 0.8fr 0.8fr 0.8fr 1fr', gap: '0', padding: '10px 0', borderBottom: i < risks.length-1 ? '1px solid #111' : 'none', alignItems: 'center' }}>
                <div><div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '600' }}>{r.name}</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{r.desc}</div></div>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{r.category}</div>
                {sevBadge(r.severity, r.sevColor)}
                <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{r.likelihood}</div>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{r.impact}</div>
                <div style={{ fontSize: '12px', fontWeight: '700', color: r.sevColor }}>{fmt(r.value)}</div>
              </div>
            ))}
          </div>
      ...
      {showBreakdownModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowBreakdownModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK SCORE BREAKDOWN</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Risk Category Analysis</div></div>
              <button onClick={() => setShowBreakdownModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {riskCategories.map((cat, i) => (
              <div key={i} style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '700' }}>{cat.label}</span>
                  </div>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <span style={{ fontSize: '11px', color: cat.dir === 'Increasing' ? '#cc4444' : '#4aaa4a', fontWeight: '600' }}>{cat.trend > 0 ? '↑ ' : '↓ '}{Math.abs(cat.trend)} {cat.dir}</span>
                    <span style={{ fontSize: '18px', fontWeight: '900', color: cat.color }}>{cat.score}/100</span>
                  </div>
                </div>
                ...
                <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '6px' }}>{cat.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {showTrendModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowTrendModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '720px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>TREND ANALYSIS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Risk Score Trends — Last 90 Days</div></div>
              <button onClick={() => setShowTrendModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '16px' }}>
              {riskCategories.slice(0,3).map((cat,i) => (
                <div key={i} style={{ padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '4px' }}>{cat.label}</div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginTop: '8px', flexWrap: 'wrap' as const }}>
              {riskCategories.map((cat,i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{cat.label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showInsightsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowInsightsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK CATEGORY INSIGHTS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Full Category Intelligence</div></div>
              <button onClick={() => setShowInsightsModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {riskCategories.map((cat, i) => (
              <div key={i} style={{ padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <div><div style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '700' }}>{cat.label}</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{cat.desc}</div></div>
                  </div>
                  <div style={{ textAlign: 'right' as const }}>
                    <div style={{ fontSize: '20px', fontWeight: '900', color: cat.color }}>{cat.score}/100</div>
                    <div style={{ fontSize: '10px', color: cat.dir === 'Increasing' ? '#cc4444' : '#4aaa4a', fontWeight: '600' }}>{cat.trend > 0 ? '↑' : '↓'} {Math.abs(cat.trend)} — {cat.dir}</div>
                  </div>
                </div>
      ...
      {showMitigationModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowMitigationModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '760px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>MITIGATION PLANS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>All Mitigation Actions</div></div>
              <button onClick={() => setShowMitigationModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {[...mitigations,
              ...
              <div key={i} style={{ display: 'flex', gap: '14px', padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '7px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: '700', marginBottom: '2px' }}>{m.title}</div>
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '6px' }}>{m.desc}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {progressBar(m.progress, m.sevColor)}
                    <span style={{ fontSize: '12px', color: 'var(--text-muted)', flexShrink: 0 }}>{m.progress}%</span>
                  </div>
                </div>
                {sevBadge(m.severity, m.sevColor)}
                <div style={{ fontSize: '11px', color: m.status === 'In Progress' ? '#4aaa4a' : '#555', fontWeight: '600', flexShrink: 0 }}>{m.status}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {showReportModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowReportModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '720px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>RISK INTELLIGENCE REPORT</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)' }}>Full Risk Intelligence Report</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{businessName} · Score: {overallRiskScore}/100 · {risks.length} risks</div></div>
            ...
            <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '12px' }}>
              <div style={{ fontSize: '10px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>EXECUTIVE RISK SUMMARY</div>
            ...
            {risks.slice(0,5).map((r, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 4 ? '1px solid #111' : 'none' }}>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                  {sevBadge(r.severity, r.sevColor)}
                  <span style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '600' }}>{r.name}</span>
                </div>
                <div style={{ fontSize: '13px', color: r.sevColor, fontWeight: '700' }}>{fmt(r.value)}</div>
              </div>
            ))}
          </div>
        </div>
