...
export default function IndustryIntelligencePage() {
  ...
  const { intelligence: liveIntelligence } = useLiveIntelligence(businessId, industryRaw)
  useEffect(() => {
    ...
    <DashboardShell activeId="industry">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '18px', borderBottom: '1px solid ' + border }}>
        <div>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Industry-specific insights, benchmarks and trends to benchmark your performance and identify opportunities.</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '10px', marginBottom: '16px' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px', padding: '14px 16px' }}>
            <div style={{ fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '8px', fontWeight: '700' }}>{k.label}</div>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '5px' }}>{k.sub}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 260px', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.15em', fontWeight: '600', marginBottom: '6px' }}>INDUSTRY OVERVIEW</div>
          <div style={{ fontSize: '22px', fontWeight: '900', color: gold, lineHeight: 1.1, marginBottom: '12px' }}>{ID.label}</div>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', lineHeight: '1.7', marginBottom: '14px' }}>{ID.description}</div>
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px', marginBottom: '12px' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: '12px', color: 'var(--text-muted)', minWidth: '85px', flexShrink: 0 }}>{m.label}</span>
                <span style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '600', lineHeight: '1.4' }}>{m.value}</span>
              </div>
            ))}
          </div>
          <div style={{ fontSize: '9px', color: 'var(--text-faint)', paddingTop: '10px', borderTop: '1px solid #1a1a1a' }}>Source: ONS, BEI Research, Companies House · Updated: 2 hours ago</div>
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>How you compare against your industry peers</div>
          ...
          <div style={{ display: 'flex', gap: '0', marginBottom: '14px', alignItems: 'stretch', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, justifyContent: 'space-between', height: '160px', paddingBottom: '24px', marginRight: '6px', flexShrink: 0 }}>
              {[100,75,50,25].map(l => <div key={l} style={{ fontSize: '9px', color: 'var(--text-secondary)', textAlign: 'right' as const }}>{l}</div>)}
            </div>
            {/* Bars */}
            <div style={{ flex: 1, display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
              {benchmarkChart.map((b, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '6px' }}>
                  ...
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)', textAlign: 'center' as const, lineHeight: 1.3, width: '100%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{b.label}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ display: 'flex', gap: '16px', marginBottom: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><div style={{ width: '10px', height: '10px', borderRadius: '2px', backgroundColor: gold }}/><span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Your Business</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><div style={{ width: '10px', height: '10px', borderRadius: '2px', backgroundColor: '#333' }}/><span style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Industry Average</span></div>
          </div>
          <div style={{ padding: '10px 12px', backgroundColor: '#080f04', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '6px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
            ...
            <div>
              <div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '700', marginBottom: '3px' }}>You outperform {beatPct}% of businesses in your industry</div>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Benchmark position: {benchmarkPosition} · Based on your MRI health score of {healthScore}/100</div>
            </div>
          </div>
        </div>
            ...
            {ID.topPerformers.map((p: any, i: number) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: i < 4 ? '1px solid #0d0d0d' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '700', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{p.name}</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{p.segment}</div>
                </div>
          ...
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '14px' }}>
            ...
            {benchmarksWithUser.slice(0, 5).map((b: any, i: number) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.6fr 0.6fr 0.6fr', gap: '0', padding: '5px 0', borderBottom: i < 4 ? '1px solid #0a0a0a' : 'none', alignItems: 'center' }}>
                <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{b.metric}</div>
                ...
                <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{typeof b.industryAvg === 'number' ? b.industryAvg + (b.unit || '') : b.industryAvg}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>Key trends shaping your industry</div>
          {ID.trends.map((t: any, i: number) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: i < ID.trends.length-1 ? '1px solid #111' : 'none' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '600' }}>{t.name}</div>
              </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>High-value opportunities in your industry</div>
            ...
            <div key={i} style={{ padding: '8px 0', borderBottom: i < ID.opportunities.length-1 ? '1px solid #111' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2px' }}>
                <div style={{ display: 'flex', gap: '7px', alignItems: 'center', flex: 1 }}>
                  ...
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '700', lineHeight: 1.2 }}>{o.name}</div>
                </div>
              ...
              <div style={{ display: 'flex', gap: '10px', paddingLeft: '29px' }}>
                ...
                <span style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{o.time}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '12px' }}>Key risks impacting your industry</div>
          {ID.risks.map((r: any, i: number) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: i < ID.risks.length-1 ? '1px solid #111' : 'none' }}>
              ...
              <div style={{ flex: 1, fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '600' }}>{r.name}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 320px', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div>
              ...
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '2px' }}>Compare key metrics across different industries</div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.7fr 0.7fr 0.8fr 0.7fr 0.9fr 0.9fr', gap: '0', padding: '6px 0', borderBottom: '1px solid #1a1a1a', marginBottom: '4px' }}>
            {['INDUSTRY','HEALTH SCORE','GROWTH RATE','PROFITABILITY','RISK LEVEL','DIGITAL MATURITY','CUSTOMER SAT.'].map(h => (
              <div key={h} style={{ fontSize: '8px', color: 'var(--text-secondary)', letterSpacing: '0.07em', fontWeight: '600' }}>{h}</div>
            ))}
          </div>
          {ID.comparisonMatrix.map((row: any, i: number) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.7fr 0.7fr 0.8fr 0.7fr 0.9fr 0.9fr', gap: '0', padding: '8px 0', borderBottom: i < ID.comparisonMatrix.length-1 ? '1px solid #0d0d0d' : 'none', alignItems: 'center', backgroundColor: i === 0 ? 'rgba(200,162,74,0.04)' : 'transparent' }}>
              ...
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{row.growth}</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{row.profitability}</div>
              <div style={{ fontSize: '12px', color: row.risk === 'High' ? '#cc4444' : row.risk === 'Medium' ? gold : '#4aaa4a', fontWeight: '600' }}>{row.risk}</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{row.digital}%</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{row.satisfaction}%</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '14px', width: '180px', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: '12px', color: 'var(--text-muted)', lineHeight: '1.6', marginBottom: '10px' }}>Industry Intelligence is calculated using algorithms analysing:</div>
          ...
          <div style={{ marginTop: '10px', paddingTop: '8px', borderTop: '1px solid #1a1a1a', fontSize: '9px', color: 'var(--text-faint)' }}>Updated daily ●</div>
          ...
          {['ONS','FCA / RICS / CIM','Bloomberg','McKinsey & Company','+15 more sources'].map((s, i) => (
            <div key={i} style={{ fontSize: '9px', color: 'var(--text-secondary)', marginBottom: '3px', paddingLeft: '4px', borderLeft: '2px solid #1a1a1a' }}>{s}</div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '14px' }}>
            ...
            {ID.insights.map((ins: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '8px', padding: '7px 0', borderBottom: i < ID.insights.length-1 ? '1px solid #0d0d0d' : 'none', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: '600', lineHeight: 1.3, marginBottom: '3px' }}>{ins.title}</div>
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{ins.time}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '14px' }}>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '8px' }}>12 month outlook</div>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '10px' }}>
              <div style={{ width: '70px', height: '70px', borderRadius: '50%', border: '3px solid ' + ID.outlookColor, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 0 16px ' + ID.outlookColor + '22' }}>
                ...
                <div style={{ fontSize: '8px', color: 'var(--text-muted)', marginTop: '2px' }}>Outlook</div>
              </div>
              <div>
            ...
            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Confidence Level: 78% · Updated: Today</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowBenchmarksModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '720px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>INDUSTRY BENCHMARKS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>All Benchmark Metrics — {ID.label}</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Your scores vs industry average vs top 25% performers</div></div>
              <button onClick={() => setShowBenchmarksModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 0.7fr 0.7fr 0.7fr 1fr', gap: '0', padding: '8px 0', borderBottom: '1px solid #1a1a1a', marginBottom: '4px' }}>
              {['METRIC','YOUR SCORE','INDUSTRY AVG','TOP 25%','YOUR POSITION'].map(h => <div key={h} style={{ fontSize: '9px', color: 'var(--text-secondary)', letterSpacing: '0.08em', fontWeight: '600' }}>{h}</div>)}
            </div>
            {benchmarksWithUser.map((b: any, i: number) => {
              ...
              return (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 0.7fr 0.7fr 0.7fr 1fr', gap: '0', padding: '12px 0', borderBottom: i < benchmarksWithUser.length-1 ? '1px solid #111' : 'none', alignItems: 'center' }}>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '600' }}>{b.metric}</div>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{typeof b.industryAvg === 'number' ? b.industryAvg + (b.unit || '') : b.industryAvg}</div>
            ...
            <div style={{ marginTop: '14px', padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: '1.6' }}>Benchmarks based on BEI Research, ONS data, and {ID.businesses} businesses in the {ID.label} sector. Your scores are derived from your Business MRI responses where available.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showPerformersModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowPerformersModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '680px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>TOP PERFORMERS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Businesses Leading the {ID.label} Sector</div></div>
              <button onClick={() => setShowPerformersModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {ID.topPerformers.map((p: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '16px', padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'center' as const }}>
                  ...
                  <div style={{ fontSize: '8px', color: 'var(--text-muted)', marginTop: '3px' }}>BEI Score</div>
                </div>
            ...
            <div style={{ padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginTop: '4px' }}>
              <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Your position: <span style={{ color: gold, fontWeight: '600' }}>{benchmarkPosition}</span> in {ID.label} · Health score: <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>{healthScore}/100</span></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showTrendsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowTrendsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>INDUSTRY TRENDS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>All Trends Shaping {ID.label}</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Trend analysis · BEI Research · Updated daily</div></div>
              <button onClick={() => setShowTrendsModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {ID.trends.map((t: any, i: number) => (
              <div key={i} style={{ padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '700' }}>{t.name}</span>
                  </div>
      ...
      {showOppsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowOppsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '740px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>EMERGING OPPORTUNITIES</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Industry Opportunities for {ID.label}</div><div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Market opportunities identified by BEI Research</div></div>
              <button onClick={() => setShowOppsModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {ID.opportunities.map((o: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '16px', padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '14px', color: 'var(--text-primary)', fontWeight: '700', marginBottom: '3px' }}>{o.name}</div>
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{o.time} · {ID.label}</div>
                </div>
      ...
      {showRisksModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowRisksModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>INDUSTRY RISK LANDSCAPE</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>All Industry Risks — {ID.label}</div></div>
              <button onClick={() => setShowRisksModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {ID.risks.map((r: any, i: number) => (
              <div key={i} style={{ padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid ' + r.levelColor + '22', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '14px', color: 'var(--text-secondary)', fontWeight: '700' }}>{r.name}</span>
                  </div>
      ...
      {showInsightsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowInsightsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>INDUSTRY INSIGHTS</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>All Industry Insights — {ID.label}</div></div>
            ...
            {ID.insights.map((ins: any, i: number) => (
              <div key={i} style={{ display: 'flex', gap: '16px', padding: '16px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid ' + ins.color + '22', marginBottom: '8px', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', color: 'var(--text-secondary)', fontWeight: '700', marginBottom: '4px', lineHeight: 1.4 }}>{ins.title}</div>
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '6px' }}>{ins.time}</div>
                  <div style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: '1.6' }}>This insight is relevant to your position in the {ID.label} sector. Current health score: {healthScore}/100 — {benchmarkPosition}.</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {showMatrixModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowMatrixModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '820px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '10px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>INDUSTRY COMPARISON MATRIX</div><div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Full Cross-Industry Comparison</div></div>
              <button onClick={() => setShowMatrixModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '18px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.7fr 0.7fr 0.8fr 0.7fr 0.9fr 0.9fr 0.9fr', gap: '0', padding: '8px 0', borderBottom: '1px solid #1a1a1a', marginBottom: '4px' }}>
              {['INDUSTRY','HEALTH SCORE','GROWTH RATE','PROFITABILITY','RISK LEVEL','DIGITAL MATURITY','CUST. SAT.','INNOVATION'].map(h => <div key={h} style={{ fontSize: '8px', color: 'var(--text-secondary)', letterSpacing: '0.07em', fontWeight: '600' }}>{h}</div>)}
            </div>
            {ID.comparisonMatrix.map((row: any, i: number) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.4fr 0.7fr 0.7fr 0.8fr 0.7fr 0.9fr 0.9fr 0.9fr', gap: '0', padding: '12px 0', borderBottom: i < ID.comparisonMatrix.length-1 ? '1px solid #111' : 'none', alignItems: 'center', backgroundColor: i === 0 ? 'rgba(200,162,74,0.04)' : 'transparent', borderRadius: i === 0 ? '6px' : '0' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{row.digital}%</div>
                </div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{row.satisfaction}%</div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{typeof row.innovation === 'number' ? <><div style={{ height: '4px', backgroundColor: '#1a1a1a', borderRadius: '2px', overflow: 'hidden', marginBottom: '2px' }}><div style={{ width: row.innovation+'%', height: '100%', backgroundColor: '#4a8ab0', opacity: 0.7, borderRadius: '2px' }} /></div><span>{row.innovation}%</span></> : '—'}</div>
              </div>
            ))}
          </div>
        </div>
