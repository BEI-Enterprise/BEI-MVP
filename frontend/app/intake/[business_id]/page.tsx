...
router.push('/processing/' + businessId)
  ...
  const inp: React.CSSProperties = {width:'100%',backgroundColor:'#111111',border:'1px solid #2a2a2a',borderRadius:'8px',padding:'12px 16px',color:'#ffffff',fontSize:'15px',outline:'none',boxSizing:'border-box',marginTop:'8px'}
  const lbl: React.CSSProperties = {display:'block',fontSize:'13px',fontWeight:'600',color:'#888888',letterSpacing:'0.05em',textTransform:'uppercase'}
      ...
      <div style={{padding:'16px 48px',backgroundColor:'#0a0a0a',borderBottom:'1px solid #1a1a1a',display:'flex',gap:'8px',alignItems:'center'}}>
        {steps.map(s => (
          <div key={s.id} style={{display:'flex',alignItems:'center',gap:'8px'}}>
            <div style={{width:'28px',height:'28px',borderRadius:'50%',backgroundColor:step===s.id?'#C8A24A':step>s.id?'#2a4a2a':'#1a1a1a',border:step===s.id?'none':step>s.id?'none':'1px solid #2a2a2a',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'12px',fontWeight:'700',color:step===s.id?'#050505':step>s.id?'#4aaa4a':'#555555'}}>
              {step > s.id ? '✓' : s.id}
            </div>
            <span style={{fontSize:'12px',color:step===s.id?'#C8A24A':step>s.id?'#4aaa4a':'#555555',fontWeight:step===s.id?'600':'400',display:'none'}}>{s.title}</span>
      ...
      <section style={{padding:'60px 48px',maxWidth:'700px',margin:'0 auto'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'28px'}}>
            {questions[step]?.map(q => (
              <div key={q.key}>
                ...
                <select value={answers[q.key]||''} onChange={e => handleAnswer(q.key, e.target.value)} style={{...inp,color:answers[q.key]?'#ffffff':'#555555'}}>
                  <option value="" disabled>Select an option</option>
                  {q.options?.map(o => <option key={o} value={o} style={{color:'#ffffff',backgroundColor:'#111111'}}>{o}</option>)}
                </select>
              </div>
            ))}
          </div>
        ...
        {step === 6 && (
          <div style={{backgroundColor:'#111111',border:'1px solid #1a1a1a',borderRadius:'12px',padding:'32px'}}>
            <div style={{fontSize:'14px',color:'#888888',marginBottom:'24px'}}>Review your answers before submitting. Your MRI will be generated immediately.</div>
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {Object.entries(answers).map(([key, value]) => (
                <div key={key} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid #1a1a1a'}}>
        ...
        <div style={{display:'flex',justifyContent:'space-between',marginTop:'48px'}}>
          {step > 1 ? (
            <button onClick={() => setStep(step-1)} style={{backgroundColor:'transparent',border:'1px solid #2a2a2a',color:'#888888',padding:'14px 28px',borderRadius:'8px',fontWeight:'600',fontSize:'14px',cursor:'pointer'}}>
              Back
            </button>
          ) : <div></div>}
          <button onClick={handleNext} disabled={loading} style={{backgroundColor:loading?'#8a6e32':'#C8A24A',color:'#050505',padding:'14px 36px',borderRadius:'8px',fontWeight:'700',fontSize:'15px',border:'none',cursor:loading?'not-allowed':'pointer'}}>
