...
export default function IntelligenceOperationsPage() {
  ...
  if (loading) return (
    <main style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.3em' }}>LOADING INTELLIGENCE OPERATIONS...</div>
    </main>
  )
  ...
  return (
    <DashboardShell activeId="operations">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '16px', borderBottom: '1px solid ' + border }}>
        <div>
          <h1 style={{ fontSize: '26px', fontWeight: '900', letterSpacing: '-0.02em', margin: '0 0 6px', color: 'var(--text-primary)' }}>Intelligence Operations™</h1>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Monitor and manage the BEI intelligence infrastructure and data operations.</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button style={{ padding: '8px 14px', backgroundColor: 'rgba(200,162,74,0.1)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '6px', color: gold, fontSize: '16px', fontWeight: '600', cursor: 'pointer' }}>⊕ Operations Scan</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '10px', marginBottom: '16px' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px', padding: '14px 16px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
            <div style={{ fontSize: '20px', color: gold, flexShrink: 0, marginTop: '2px' }}>{k.icon}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: '4px', fontWeight: '600' }}>{k.label}</div>
              ...
              <div style={{ fontSize: '16px', color: k.subColor, fontWeight: '600', marginBottom: '3px' }}>{k.sub}</div>
              <div style={{ fontSize: '16px', color: k.trendColor }}>{k.trend}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>Data Pipeline Flow</div>
            <button onClick={() => setShowPipelineModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View pipeline →</button>
          </div>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '18px' }}>Real-time view of your data pipeline</div>
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '8px' }}>
                  ...
                  <div style={{ textAlign: 'center' as const }}>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '3px' }}>{s.label}</div>
                    <div style={{ fontSize: '20px', fontWeight: '800', color: s.active ? s.color : '#333', lineHeight: 1 }}>{s.metric}</div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{s.sub}</div>
                  </div>
                </div>
              )
            })}
          ...
          <div style={{ display: 'flex', gap: '20px', padding: '10px 14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Throughput: <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>{processingSpeed.toLocaleString()} records/min</span></span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: sourcesCount > 0 ? '#4aaa4a' : '#333' }} />
              <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Latency: <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>{latency}s</span></span>
            </div>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>System Health</div>
            <button onClick={() => setShowSystemModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View system health →</button>
          </div>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '14px' }}>Overall system and infrastructure health</div>
              ...
              {systemComponents.map((comp, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderBottom: i < systemComponents.length - 1 ? '1px solid #0d0d0d' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                    ...
                    <span style={{ fontSize: '16px', color: 'var(--text-secondary)' }}>{comp.name}</span>
                  </div>
                  <span style={{ fontSize: '16px', color: comp.color, fontWeight: '600', maxWidth: '170px', textAlign: 'right' as const, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{comp.status}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>Intelligence Engine Performance</div>
            <button onClick={() => setShowPerformanceModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View performance →</button>
          </div>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '14px' }}>Performance metrics over the last 24 hours</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '8px', marginBottom: '14px' }}>
            {[
              ...
              <div key={i} style={{ padding: '10px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '7px', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: '4px', fontWeight: '600' }}>{m.label}</div>
          ...
          {(headcount || utilisation || onTimePct || automationPct) && (
            <div style={{ marginTop: '10px', padding: '10px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '7px', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.1em', marginBottom: '6px', fontWeight: '600' }}>YOUR OPERATIONAL DATA</div>
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' as const }}>
                {headcount && <div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Headcount</div><div style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '700' }}>{headcount}</div></div>}
                {utilisation && <div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Utilisation</div><div style={{ fontSize: '16px', color: utilisation >= 85 ? '#e8923a' : '#4aaa4a', fontWeight: '700' }}>{utilisation}%</div></div>}
                {onTimePct && <div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>On-Time</div><div style={{ fontSize: '16px', color: onTimePct >= 80 ? '#4aaa4a' : gold, fontWeight: '700' }}>{onTimePct}%</div></div>}
                {automationPct && <div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Automation</div><div style={{ fontSize: '16px', color: gold, fontWeight: '700' }}>{automationPct}%</div></div>}
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>Recent Alerts</div>
            <button onClick={() => setShowAlertsModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View all alerts →</button>
          </div>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '14px' }}>Live system alerts from your BEI environment</div>
          {alerts.length === 0 ? (
            <div style={{ textAlign: 'center' as const, padding: '24px', color: 'var(--text-faint)', fontSize: '16px' }}>Complete your MRI to activate monitoring</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '7px' }}>
              {alerts.slice(0, 5).map((a, i) => (
                <div key={i} style={{ display: 'flex', gap: '9px', padding: '9px 11px', backgroundColor: a.color + '08', borderRadius: '7px', border: '1px solid ' + a.color + '25', alignItems: 'flex-start' }}>
                  <div style={{ fontSize: '16px', color: a.color, flexShrink: 0, marginTop: '1px' }}>
                    {a.level === 'success' ? '✓' : a.level === 'info' ? 'ℹ' : '⚠'}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600', marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{a.title}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{a.area}</span>
                      <span style={{ fontSize: '16px', color: 'var(--text-secondary)' }}>{a.time}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '18px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
            <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)' }}>Recent Integrations</div>
            <button onClick={() => setShowIntegrationsModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View all integrations →</button>
          </div>
          <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '14px' }}>Connected data sources for this business</div>
          {recentIntegrations.length === 0 ? (
            <div style={{ textAlign: 'center' as const, padding: '20px' }}>
              <div style={{ fontSize: '16px', color: 'var(--text-faint)', marginBottom: '10px' }}>No integrations connected yet</div>
              <a href="/connect" style={{ fontSize: '16px', color: gold, textDecoration: 'none', fontWeight: '600' }}>→ Add your first data source</a>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '7px' }}>
              {recentIntegrations.map((int, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '9px', padding: '9px 11px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '7px', border: '1px solid var(--border)' }}>
                  <div style={{ width: '30px', height: '30px', borderRadius: '7px', backgroundColor: int.statusColor + '18', border: '1px solid ' + int.statusColor + '33', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '16px', color: int.statusColor, fontWeight: '700' }}>
                    {int.name.charAt(0).toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{int.name}</div>
                    <div style={{ fontSize: '16px', color: int.statusColor, fontWeight: '600' }}>{int.status}</div>
                  </div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', textAlign: 'right' as const, flexShrink: 0 }}>{int.time}</div>
                </div>
              ))}
            </div>
          )}
          <div style={{ marginTop: '10px', paddingTop: '10px', borderTop: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', fontSize: '16px', color: 'var(--text-muted)' }}>
        ...
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowAlertsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>ALL ALERTS</div><div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)' }}>System Alerts — {businessName}</div></div>
              <button onClick={() => setShowAlertsModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
              ...
              <div key={i} style={{ padding: '14px', backgroundColor: a.color + '08', borderRadius: '8px', border: '1px solid ' + a.color + '25', marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '5px' }}>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <span style={{ fontSize: '16px', color: a.color }}>{a.level === 'success' ? '✓' : a.level === 'info' ? 'ℹ' : '⚠'}</span>
                    <span style={{ fontSize: '16px', color: 'var(--text-primary)', fontWeight: '700' }}>{a.title}</span>
                  </div>
                  <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{a.time}</span>
                </div>
                <div style={{ fontSize: '16px', color: a.color, fontWeight: '600', marginBottom: '4px' }}>{a.area}</div>
      ...
      {showIntegrationsModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowIntegrationsModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>ALL INTEGRATIONS</div><div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Connected Data Sources</div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{totalConns} total · {activeConns.length} active · {errorConns.length} errors</div></div>
              <button onClick={() => setShowIntegrationsModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {safeConnectors.length === 0 ? (
              <div style={{ textAlign: 'center' as const, padding: '30px' }}>
                <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '10px' }}>No integrations connected</div>
                <a href="/connect" style={{ padding: '10px 20px', backgroundColor: gold, color: '#050505', borderRadius: '6px', textDecoration: 'none', fontSize: '16px', fontWeight: '700' }}>Connect data sources →</a>
              </div>
              ...
              <div key={i} style={{ display: 'flex', gap: '14px', padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '8px', alignItems: 'center' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '8px', backgroundColor: (c.status === 'active' ? '#4aaa4a' : c.status === 'error' ? '#cc4444' : '#555') + '18', border: '1px solid ' + (c.status === 'active' ? '#4aaa4a' : c.status === 'error' ? '#cc4444' : '#555') + '33', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '16px', color: c.status === 'active' ? '#4aaa4a' : c.status === 'error' ? '#cc4444' : '#888', fontWeight: '700' }}>
                  {(c.connector_name || '?').charAt(0).toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '16px', color: 'var(--text-primary)', fontWeight: '700', marginBottom: '2px' }}>{c.connector_name}</div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Type: {c.connector_type} · Sync every {c.sync_frequency_hours || 24}h</div>
                  {c.error_message && <div style={{ fontSize: '16px', color: '#cc4444', marginTop: '2px' }}>{c.error_message}</div>}
                </div>
                <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
                  <div style={{ fontSize: '16px', color: c.status === 'active' ? '#4aaa4a' : c.status === 'error' ? '#cc4444' : '#888', fontWeight: '700', marginBottom: '2px' }}>{c.status === 'active' ? 'Connected' : c.status === 'error' ? 'Error' : 'Pending'}</div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{c.last_synced_at ? 'Synced ' + new Date(c.last_synced_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : 'Not yet synced'}</div>
                </div>
              </div>
            ))}
            <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'center' }}>
              <a href="/connect" style={{ padding: '10px 20px', backgroundColor: 'rgba(200,162,74,0.08)', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '6px', color: gold, textDecoration: 'none', fontSize: '16px', fontWeight: '600' }}>Manage all integrations →</a>
            </div>
          </div>
        </div>
      )}
      ...
      {showSystemModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowSystemModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>SYSTEM HEALTH</div><div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Full System Health Report</div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Overall: {systemHealthPct.toFixed(1)}% — {systemHealthLabel}</div></div>
              <button onClick={() => setShowSystemModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {systemComponents.map((comp, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid ' + comp.color + '22', marginBottom: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '700' }}>{comp.name}</span>
                </div>
                <span style={{ fontSize: '16px', color: comp.color, fontWeight: '600' }}>{comp.status}</span>
              </div>
            ))}
            <div style={{ padding: '12px 14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginTop: '4px' }}>
              <div style={{ fontSize: '16px', color: 'var(--text-muted)', lineHeight: '1.7' }}>System health of {systemHealthPct.toFixed(1)}% is calculated from MRI completion, active connectors ({activeConns.length}), and error states ({errorConns.length}). Add more data sources to improve your score.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showPipelineModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowPipelineModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '680px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>DATA PIPELINE</div><div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Pipeline Detail</div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{sourcesCount} source(s) · {fmtNum(dataPoints)} data points</div></div>
              <button onClick={() => setShowPipelineModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '16px' }}>
              {[
                ...
                <div key={i} style={{ padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '5px', fontWeight: '600' }}>{m.label}</div>
            ...
            <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.1em', marginBottom: '8px', fontWeight: '600' }}>ACTIVE DATA SOURCES</div>
              {!hasMRI && activeConns.length === 0 && <div style={{ fontSize: '16px', color: 'var(--text-secondary)', textAlign: 'center' as const, padding: '10px' }}>Complete your Business MRI to activate the data pipeline.</div>}
              {hasMRI && (
                <div style={{ display: 'flex', gap: '8px', padding: '8px 10px', backgroundColor: 'var(--bg-card)', borderRadius: '6px', marginBottom: '5px', alignItems: 'center' }}>
                  ...
                  <span style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600', flex: 1 }}>Business MRI (Primary Source)</span>
                  <span style={{ fontSize: '16px', color: '#4aaa4a' }}>Active</span>
                </div>
              )}
              {activeConns.map((c, i) => (
                <div key={i} style={{ display: 'flex', gap: '8px', padding: '8px 10px', backgroundColor: 'var(--bg-card)', borderRadius: '6px', marginBottom: '4px', alignItems: 'center' }}>
                  <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: '#4aaa4a' }} />
                  <span style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600', flex: 1 }}>{c.connector_name}</span>
                  <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{c.last_synced_at ? new Date(c.last_synced_at).toLocaleDateString('en-GB') : 'Pending'}</span>
                  <span style={{ fontSize: '16px', color: '#4aaa4a', fontWeight: '600' }}>Active</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {showPerformanceModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowPerformanceModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div><div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>ENGINE PERFORMANCE</div><div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Intelligence Engine Report</div><div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Accuracy: {engineAccuracy > 0 ? engineAccuracy + '%' : 'Awaiting MRI'} · Confidence: {confidence.toUpperCase()}</div></div>
              <button onClick={() => setShowPerformanceModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '16px' }}>
              {[
                ...
                <div key={i} style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '600' }}>{m.label}</div>
            ...
            {hasMRI && (
              <div style={{ padding: '14px', backgroundColor: '#080f04', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '8px', marginBottom: '12px' }}>
                <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.1em', marginBottom: '8px', fontWeight: '600' }}>MRI INTELLIGENCE SUMMARY</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <div><div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '2px' }}>Primary Constraint</div><div style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600' }}>{primary?.name || 'None detected'}</div></div>
                  <div><div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '2px' }}>Confidence Level</div><div style={{ fontSize: '16px', color: confidence === 'high' ? '#4aaa4a' : gold, fontWeight: '600' }}>{confidence.toUpperCase()}</div></div>
                  <div><div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '2px' }}>Health Score</div><div style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '600' }}>{healthScore}/100</div></div>
                  <div><div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '2px' }}>Operations Pillar</div><div style={{ fontSize: '16px', color: opsScore ? (opsScore >= 70 ? '#4aaa4a' : gold) : '#555', fontWeight: '600' }}>{opsScore ? opsScore + '/100' : 'N/A'}</div></div>
                </div>
              </div>
            )}
            <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
              <div style={{ fontSize: '16px', color: 'var(--text-muted)', lineHeight: '1.7' }}>Engine accuracy derives from MRI confidence ({confidence}) and {activeConns.length} connected source(s). More connected sources increase accuracy. {!hasMRI && 'Complete your Business MRI to activate the intelligence engine.'}</div>
            </div>
          </div>
        </div>
      )}
