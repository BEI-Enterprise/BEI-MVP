...
export default function OpportunitiesPage() {
  ...
  if (loading) return (
    <main style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.3em' }}>LOADING OPPORTUNITY INTELLIGENCE...</div>
    </main>
  )
  ...
  const impBadge = (level: string, color: string) => (
    <div style={{ padding: '3px 8px', backgroundColor: color + '18', border: '1px solid ' + color + '44', borderRadius: '4px', fontSize: '17px', color, fontWeight: '700', whiteSpace: 'nowrap' as const, display: 'inline-block', width: 'fit-content' }}>{level}</div>
  )
  ...
  return (
    <DashboardShell activeId="opportunities">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid ' + border }}>
        <div>
          <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.25em', marginBottom: '6px', fontWeight: '600' }}>OPPORTUNITY CENTRE™</div>
          <h1 style={{ fontSize: '27px', fontWeight: '900', letterSpacing: '-0.02em', margin: '0 0 6px', color: 'var(--text-primary)', lineHeight: 1.1 }}>Discover, prioritise and realise your highest-value opportunities.</h1>
          <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>AI-powered opportunity intelligence based on your business data, constraints and performance.</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0, marginLeft: '20px' }}>
          ...
          <button onClick={() => setShowReportModal(true)} style={{ padding: '9px 16px', backgroundColor: gold, border: 'none', borderRadius: '6px', color: '#050505', fontSize: '17px', fontWeight: '700', cursor: 'pointer', whiteSpace: 'nowrap' as const }}>View Full Opportunity Report →</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '10px', marginBottom: '16px' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px', padding: '14px 16px' }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '7px', fontWeight: '700' }}>{k.label}</div>
            <div style={{ fontSize: '27px', fontWeight: '900', color: k.color, lineHeight: 1, marginBottom: '4px' }}>{k.value}</div>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '6px' }}>{k.sub}</div>
            <div style={{ fontSize: '17px', color: '#4aaa4a', fontWeight: '600' }}>{k.trend}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '210px 1fr 270px', gap: '12px', marginBottom: '16px' }}>
        ...
        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
            <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>FILTER OPPORTUNITIES</div>
            <button onClick={() => { setFilterImpact([]); setFilterTime('All Timeframes'); setFilterType('All Types'); setFilterArea('All Areas'); setFilterConfidence(0); setSearchTerm('') }} style={{ fontSize: '17px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Clear all</button>
          </div>
          <div style={{ marginBottom: '14px' }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '8px', fontWeight: '600' }}>Impact Level</div>
            {['Critical','High','Medium','Low'].map(level => {
              ...
              return (
                <div key={level} onClick={() => setFilterImpact(prev => checked ? prev.filter(x => x !== level) : [...prev, level])} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 0', cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: '17px', color: 'var(--text-secondary)' }}>{level}</span>
                </div>
              )
            })}
          </div>
          <div style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '600' }}>Time to Realise</div>
            <select value={filterTime} onChange={e => setFilterTime(e.target.value)} style={{ width: '100%', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '5px', color: 'var(--text-secondary)', fontSize: '17px', padding: '7px 8px', cursor: 'pointer' }}>
              {['All Timeframes','1-3 months','3-6 months','6-12 months','12+ months'].map(o => <option key={o}>{o}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '600' }}>Opportunity Type</div>
            <select value={filterType} onChange={e => setFilterType(e.target.value)} style={{ width: '100%', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '5px', color: 'var(--text-secondary)', fontSize: '17px', padding: '7px 8px', cursor: 'pointer' }}>
              {['All Types','Growth','Operations','Finance','Client','Risk'].map(o => <option key={o}>{o}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '600' }}>Affected Areas</div>
            <select value={filterArea} onChange={e => setFilterArea(e.target.value)} style={{ width: '100%', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '5px', color: 'var(--text-secondary)', fontSize: '17px', padding: '7px 8px', cursor: 'pointer' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
              <div style={{ fontSize: '17px', color: 'var(--text-muted)', fontWeight: '600' }}>Confidence</div>
              <span style={{ fontSize: '17px', color: gold }}>{filterConfidence}%</span>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2px' }}>
              <span style={{ fontSize: '17px', color: 'var(--text-secondary)' }}>0%</span><span style={{ fontSize: '17px', color: 'var(--text-secondary)' }}>100%</span>
            </div>
          </div>
          <div>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '600' }}>Search Opportunities</div>
            <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search by name..." style={{ width: '100%', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '5px', color: 'var(--text-secondary)', fontSize: '17px', padding: '7px 8px', boxSizing: 'border-box' as const }} />
          </div>
        </div>

        <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
          <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600', marginBottom: '12px' }}>OPPORTUNITY PIPELINE</div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 0.6fr 1fr 0.7fr 0.9fr 0.6fr 0.9fr 55px 20px', gap: '0', paddingBottom: '7px', borderBottom: '1px solid #1a1a1a', marginBottom: '2px' }}>
            {['OPPORTUNITY','TYPE','AFFECTED AREAS','IMPACT','VALUE','CONF.','TIME','TREND',''].map(h => (
              <div key={h} style={{ fontSize: '17px', color: 'var(--text-muted)', letterSpacing: '0.08em', fontWeight: '600' }}>{h}</div>
            ))}
          </div>
          {pagedOpps.map((opp, i) => (
            <div key={opp.id} onClick={() => setSelectedOpp(opp)} style={{ display: 'grid', gridTemplateColumns: '2fr 0.6fr 1fr 0.7fr 0.9fr 0.6fr 0.9fr 55px 20px', gap: '0', padding: '9px 0', borderBottom: i < pagedOpps.length - 1 ? '1px solid #111' : 'none', alignItems: 'center', cursor: 'pointer' }}>
              <div style={{ display: 'flex', gap: '7px', alignItems: 'center' }}>
                <div style={{ width: '26px', height: '26px', borderRadius: '6px', backgroundColor: opp.iconBg + '22', border: '1px solid ' + opp.iconBg + '44', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '17px', color: opp.iconBg }}>{opp.icon}</div>
                <div>
                  <div style={{ fontSize: '17px', color: 'var(--text-secondary)', fontWeight: '700' }}>{opp.name}</div>
                  <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{opp.desc}</div>
                </div>
              </div>
              <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{opp.type}</div>
              <div style={{ display: 'flex', gap: '2px', flexWrap: 'wrap' as const }}>
                {opp.areas.slice(0,2).map(a => (
                  <div key={a} style={{ padding: '1px 4px', backgroundColor: 'rgba(200,162,74,0.08)', border: '1px solid rgba(200,162,74,0.15)', borderRadius: '3px', fontSize: '17px', color: 'var(--text-muted)' }}>{a}</div>
                ))}
              </div>
              {impBadge(opp.impact, opp.impactColor)}
              <div>
                <div style={{ fontSize: '17px', color: 'var(--text-secondary)', fontWeight: '700' }}>{fmt(opp.value)}</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Annual</div>
              </div>
              {confCircle(opp.confidence, opp.confidence >= 80 ? '#4aaa4a' : opp.confidence >= 65 ? gold : '#e8923a')}
              <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{opp.timeToRealise}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px', paddingTop: '10px', borderTop: '1px solid ' + border }}>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Showing {Math.min((currentPage-1)*perPage+1, filteredOpps.length)}–{Math.min(currentPage*perPage, filteredOpps.length)} of {filteredOpps.length}</div>
            <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
              <button onClick={() => setCurrentPage(p => Math.max(1,p-1))} disabled={currentPage===1} style={{ width: '26px', height: '26px', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '4px', color: currentPage===1 ? '#333' : '#888', cursor: currentPage===1 ? 'default' : 'pointer', fontSize: '17px' }}>‹</button>
              {Array.from({length: totalPages}, (_,i) => i+1).map(p => (
                <button key={p} onClick={() => setCurrentPage(p)} style={{ width: '26px', height: '26px', backgroundColor: p===currentPage ? gold : '#0a0a0a', border: '1px solid ' + (p===currentPage ? gold : border), borderRadius: '4px', color: p===currentPage ? '#050505' : '#888', cursor: 'pointer', fontSize: '17px', fontWeight: p===currentPage ? '700' : '400' }}>{p}</button>
              ))}
              <button onClick={() => setCurrentPage(p => Math.min(totalPages,p+1))} disabled={currentPage===totalPages} style={{ width: '26px', height: '26px', backgroundColor: 'var(--bg-sidebar)', border: '1px solid ' + border, borderRadius: '4px', color: currentPage===totalPages ? '#333' : '#888', cursor: currentPage===totalPages ? 'default' : 'pointer', fontSize: '17px' }}>›</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '12px' }}>
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>OPPORTUNITY BY TYPE</div>
              <button onClick={() => setShowTypeModal(true)} style={{ fontSize: '17px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View full →</button>
            </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                {typeBreakdown.slice(0,5).map((o, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
                    ...
                    <div style={{ flex: 1, fontSize: '17px', color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{o.label}</div>
                    <div style={{ fontSize: '17px', color: o.color, fontWeight: '600', flexShrink: 0 }}>{o.pct}%</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>VALUE OVER TIME</div>
              <button onClick={() => setShowForecastModal(true)} style={{ fontSize: '17px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>View forecast →</button>
            </div>
            <div style={{ fontSize: '17px', fontWeight: '800', color: gold, marginBottom: '2px' }}>{fmt(totalValue)}</div>
            <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '8px' }}>Total forecasted value</div>
            <div style={{ display: 'flex', gap: '10px', marginBottom: '6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><div style={{ width: '8px', height: '3px', backgroundColor: gold, borderRadius: '2px' }}/><span style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Forecast</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><div style={{ width: '8px', height: '3px', backgroundColor: '#4aaa4a', borderRadius: '2px' }}/><span style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Achieved</span></div>
            </div>
          ...
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>OPPORTUNITY MATURITY</div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              {maturityStages.map((s, i) => <div key={i} style={{ fontSize: '17px', color: s.color, fontWeight: '600' }}>{fmt(s.val)}</div>)}
            </div>
            <div style={{ fontSize: '17px', color: 'var(--text-secondary)', marginTop: '6px' }}>Avg time to realise: 4.2 months</div>
          </div>
        </div>
      </div>

      <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '16px' }}>
        <div style={{ fontSize: '17px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600', marginBottom: '12px' }}>TOP OPPORTUNITY HIGHLIGHTS</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '10px' }}>
          {[
            ...
            <div key={i} style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid ' + border }}>
              <div style={{ fontSize: '17px', color: h.badgeColor, letterSpacing: '0.1em', fontWeight: '700', marginBottom: '8px' }}>{h.badge}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '8px' }}>
                <div style={{ width: '26px', height: '26px', borderRadius: '6px', backgroundColor: h.iconBg + '22', border: '1px solid ' + h.iconBg + '44', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', color: h.iconBg }}>{h.icon}</div>
                <div style={{ fontSize: '17px', color: 'var(--text-primary)', fontWeight: '700', lineHeight: 1.2 }}>{h.name}</div>
              </div>
              <div style={{ fontSize: '22px', fontWeight: '900', color: h.badgeColor, lineHeight: 1, marginBottom: '2px' }}>{h.metric}</div>
              <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '3px' }}>{h.metricSub}</div>
              <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{h.extra}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setSelectedOpp(null)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '700px', maxWidth: '95vw', marginBottom: '40px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>OPPORTUNITY DETAIL</div>
                <div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>{selectedOpp.name}</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{selectedOpp.desc} · Type: {selectedOpp.type}</div>
              </div>
              <button onClick={() => setSelectedOpp(null)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '22px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
              <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>AFFECTED AREAS</div>
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
                  {selectedOpp.areas.map((a: string) => (
                    <div key={a} style={{ padding: '4px 10px', backgroundColor: 'rgba(200,162,74,0.08)', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '4px', fontSize: '17px', color: gold, fontWeight: '600' }}>{a}</div>
                  ))}
                </div>
              </div>
              <div style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>MATURITY STATUS</div>
                <div style={{ fontSize: '17px', fontWeight: '700', color: 'var(--text-secondary)', marginBottom: '4px' }}>{selectedOpp.maturity}</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Current stage in opportunity pipeline</div>
              </div>
            </div>
            <div style={{ padding: '14px', backgroundColor: '#080f04', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '8px', marginBottom: '14px' }}>
              <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>HOW TO REALISE THIS OPPORTUNITY</div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={() => setSelectedOpp(null)} style={{ flex: 1, padding: '10px', backgroundColor: gold, color: '#050505', border: 'none', borderRadius: '6px', fontWeight: '700', fontSize: '17px', cursor: 'pointer' }}>Deploy Resolution Framework →</button>
              <button onClick={() => setSelectedOpp(null)} style={{ padding: '10px 20px', backgroundColor: 'transparent', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', fontSize: '17px', cursor: 'pointer' }}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showReportModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowReportModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '820px', maxWidth: '95vw', marginBottom: '40px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>FULL OPPORTUNITY REPORT</div>
                <div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Complete Opportunity Intelligence</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{businessName} · Total: {fmt(totalValue)} · {opportunities.length} opportunities identified</div>
              </div>
            ...
            {opportunities.map((opp, i) => (
              <div key={i} style={{ display: 'flex', gap: '14px', padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginBottom: '7px', alignItems: 'center' }}>
                <div style={{ width: '30px', height: '30px', borderRadius: '6px', backgroundColor: opp.iconBg + '22', border: '1px solid ' + opp.iconBg + '44', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '17px', color: opp.iconBg }}>{opp.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: '17px', color: 'var(--text-primary)', fontWeight: '700', marginBottom: '2px' }}>{opp.name}</div>
                      <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{opp.desc} · {opp.type} · {opp.timeToRealise}</div>
                    </div>
                    <div style={{ textAlign: 'right' as const, flexShrink: 0, marginLeft: '16px' }}>
                      <div style={{ fontSize: '17px', fontWeight: '800', color: gold }}>{fmt(opp.value)}</div>
                      <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>{opp.confidence}% confidence</div>
                    </div>
                  </div>
                </div>
                {impBadge(opp.impact, opp.impactColor)}
      ...
      {showForecastModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowForecastModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '680px', maxWidth: '95vw', marginBottom: '40px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>VALUE FORECAST</div>
                <div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>6-Month Value Realisation Forecast</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Projected value unlock based on deployment timeline</div>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '10px', marginBottom: '16px' }}>
              {[
                ...
                <div key={i} style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '17px', color: 'var(--text-muted)', marginBottom: '5px' }}>{m.month}</div>
                  <div style={{ fontSize: '22px', fontWeight: '800', color: m.color, marginBottom: '3px' }}>{m.value}</div>
            ...
            <div style={{ padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', marginTop: '12px' }}>
              <div style={{ fontSize: '17px', color: 'var(--text-muted)', lineHeight: '1.6' }}>Forecast assumes deployment of Tier 1 actions within 30 days. Achieved line represents conservative 70% realisation rate based on industry benchmarks.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showTypeModal && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.88)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowTypeModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '660px', maxWidth: '95vw', marginBottom: '40px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '17px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>OPPORTUNITY BY TYPE</div>
                <div style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Full Type Breakdown</div>
                <div style={{ fontSize: '17px', color: 'var(--text-muted)' }}>All opportunity types · Total: {fmt(totalValue)}</div>
              </div>
              <button onClick={() => setShowTypeModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '22px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {typeBreakdown.map((o, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.5fr 50px 2fr 80px', gap: '12px', alignItems: 'center', padding: '12px 0', borderBottom: i < typeBreakdown.length-1 ? '1px solid #111' : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '17px', color: 'var(--text-secondary)', fontWeight: '600' }}>{o.label}</span>
                </div>
                <div style={{ fontSize: '17px', color: o.color, fontWeight: '700' }}>{o.pct}%</div>
                ...
                <div style={{ fontSize: '17px', color: o.color, fontWeight: '700', textAlign: 'right' as const }}>{fmt(o.val)}</div>
              </div>
            ))}
            <div style={{ marginTop: '14px', padding: '12px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '17px', color: 'var(--text-muted)' }}>Total opportunity value</span>
              <span style={{ fontSize: '17px', fontWeight: '800', color: gold }}>{fmt(totalValue)}</span>
            </div>
          </div>
        </div>
      )}
