...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (authError) {
      setLoading(false)
      if (authError.message.includes('Invalid login') || authError.message.includes('user')) {
    ...
    <main style={{backgroundColor:'#050505',color:'#fff',fontFamily:'Inter,system-ui,sans-serif',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{width:'100%',maxWidth:'420px',padding:'0 24px'}}>
        ...
        <div style={{padding:'40px',border:'1px solid #1a1a1a',borderRadius:'12px',backgroundColor:'#080808'}}>
          <div style={{fontSize:'11px',color:gold,letterSpacing:'0.2em',textTransform:'uppercase' as const,marginBottom:'8px'}}>Intelligence Platform</div>
          ...
          <div style={{marginBottom:'16px'}}>
            ...
            <input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="you@company.com"
              ...
              style={{width:'100%',padding:'12px 14px',backgroundColor:'#0d0d0d',border:'1px solid #2a2a2a',borderRadius:'6px',color:'#fff',fontSize:'14px',outline:'none',boxSizing:'border-box' as const}} />
          </div>
          ...
          {error && (
            <div style={{padding:'12px 14px',backgroundColor:'#1a0a0a',border:'1px solid #cc4444',borderRadius:'6px',fontSize:'13px',color:'#cc4444',marginBottom:'16px',lineHeight:'1.6'}}>
              ...
              {error.includes('No account') && (
                <a href="/register" style={{color:gold,textDecoration:'none',fontWeight:'600'}}>Request access →</a>
              )}
            </div>
          )}
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:'14px',backgroundColor:gold,color:'#050505',fontWeight:'700',borderRadius:'6px',border:'none',cursor:'pointer',fontSize:'15px',opacity:loading?0.7:1}}>
            {loading ? 'Signing in...' : 'Sign in →'}
          </button>
          <div style={{marginTop:'24px',textAlign:'center' as const,fontSize:'13px',color:'#555'}}>
