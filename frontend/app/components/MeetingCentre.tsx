...
export default function MeetingCentre({ gold, card, border, dark, businessId }: { gold: string, card: string, border: string, dark: string, businessId?: string }) {
  ...
  return (
    <div>
      <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>BEI MEETING EXECUTION CENTRE</div>
      <div style={{ fontSize: '13px', color: '#aaa', marginBottom: '24px', lineHeight: '1.7' }}>Book sessions with your BEI Intelligence specialist. Add meeting notes so your account manager arrives fully prepared.</div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>

        {/* Calendar */}
        <div style={{ padding: '24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px' }}>
          <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>
            {monthNames[today.getMonth()].toUpperCase()} {today.getFullYear()}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px', marginBottom: '6px' }}>
            {['Mo','Tu','We','Th','Fr','Sa','Su'].map((d) => (
              <div key={d} style={{ fontSize: '10px', color: '#888', textAlign: 'center' as const, padding: '4px 0', fontWeight: '600' }}>{d}</div>
            ))}
          </div>
          ...
          <div style={{ marginTop: '16px', paddingTop: '12px', borderTop: '1px solid #222', fontSize: '12px', color: '#888', display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        {/* Session types */}
        <div style={{ padding: '24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px' }}>
          <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>AVAILABLE SESSIONS</div>
            ...
            {meetingTypes.map(m => (
              <button key={m.id} onClick={() => setSelectedMeeting(selectedMeeting === m.id ? '' : m.id)} style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: '11px', color: '#999' }}>{m.desc}</div>
                </div>
                <div style={{ fontSize: '10px', color: '#888', flexShrink: 0 }}>{m.duration}</div>
              </button>
            ))}
          </div>
          {selectedMeeting && (
      ...
      <div style={{ padding: '24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', marginBottom: '16px' }}>
        <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>MEETING NOTES</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: '12px', color: '#888' }}>Notes are visible to your BEI account manager only.</div>
      ...
      <div style={{ padding: '20px 24px', backgroundColor: '#141414', border: '1px solid #2a2a2a', borderRadius: '8px', display: 'flex', gap: '16px', alignItems: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '12px', color: '#999' }}>All sessions conducted by qualified BEI Intelligence specialists. Direct contact: <span style={{ color: gold }}>intelligence@officialbei.com</span> · Response within 24 hours.</div>
        </div>
      </div>
    </div>
  )
