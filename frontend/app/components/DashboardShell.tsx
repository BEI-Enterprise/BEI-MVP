    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ width: collapsed ? '60px' : '240px', minHeight: '100vh', backgroundColor: sidebar, borderRight: '1px solid var(--sidebar-border)', display: 'flex', flexDirection: 'column' as const, position: 'fixed' as const, top: 0, left: 0, bottom: 0, zIndex: 50, transition: 'width 0.2s ease', overflowX: 'hidden' as const, overflowY: 'auto' as const }}>
        ...
        <div style={{ padding: collapsed ? '18px 0' : '18px 18px', borderBottom: '1px solid var(--sidebar-border)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', minHeight: '60px', flexShrink: 0 }}>
          {!collapsed && <div><div style={{ fontSize: '22px', fontWeight: '900', color: gold, letterSpacing: '0.15em' }}>BEI<sup style={{ fontSize: '17px', verticalAlign: 'super' }}>TM</sup></div><div style={{ fontSize: '17px', color: 'var(--text-secondary)', letterSpacing: '0.18em', marginTop: '2px' }}>BUSINESS EXECUTION INTELLIGENCE</div></div>}
          <button onClick={() => setCollapsed(!collapsed)} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: '17px', padding: '4px', flexShrink: 0 }}>{collapsed ? '▶' : '◀'}</button>
        </div>
        ...
        <nav style={{ flex: 1, padding: '8px 0', overflowY: 'auto' as const }}>
          {navSections.map(({ section, items }) => (
            <div key={section}>
              {!collapsed && <div style={{ fontSize: '17px', color: 'var(--text-faint)', letterSpacing: '0.2em', fontWeight: '700', padding: '12px 18px 5px', textTransform: 'uppercase' as const }}>{section}</div>}
              {items.map(item => {
                const isActive = activeId === item.id
                const Icon = icons[item.id]
                return (
                  <a key={item.id} href={item.href} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: collapsed ? '10px 0' : '9px 16px', justifyContent: collapsed ? 'center' : 'flex-start', textDecoration: 'none', backgroundColor: isActive ? 'rgba(200,162,74,0.08)' : 'transparent', borderLeft: isActive ? `2px solid ${gold}` : '2px solid transparent', color: isActive ? gold : '#777', fontSize: '17px', fontWeight: isActive ? '600' : '400', transition: 'all 0.15s', whiteSpace: 'nowrap' as const }}
            ...
            <button onClick={() => window.dispatchEvent(new CustomEvent('open-ask-bei'))} style={{ width: '100%', backgroundColor: 'rgba(200,162,74,0.05)', border: '1px solid rgba(200,162,74,0.15)', borderRadius: '10px', padding: '11px 13px', cursor: 'pointer', textAlign: 'left' as const, display: 'block' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '5px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <span style={{ fontSize: '17px', color: gold }}>✦</span>
                  <span style={{ fontSize: '17px', fontWeight: '700', color: gold, letterSpacing: '0.05em' }}>BEI ASSISTANT</span>
                </div>
                <div style={{ width: '20px', height: '20px', borderRadius: '5px', backgroundColor: 'rgba(200,162,74,0.12)', border: '1px solid rgba(200,162,74,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', color: gold }}>→</div>
              </div>
              <div style={{ fontSize: '17px', color: 'var(--text-secondary)', lineHeight: '1.5' }}>Ask BEI anything about your Business Twin...</div>
            </button>
          </div>
        )}
        ...
        {!collapsed && (
          <div style={{ padding: '10px 16px', borderTop: '1px solid var(--sidebar-border)', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              ...
              <span style={{ fontSize: '17px', color: '#4aaa4a', fontWeight: '600', letterSpacing: '0.12em' }}>INTELLIGENCE ACTIVE</span>
            </div>
            <div style={{ fontSize: '17px', color: 'var(--text-faint)', marginTop: '2px' }}>All systems operational</div>
          </div>
        )}
      </div>
      ...
      <div style={{ marginLeft: collapsed ? '60px' : '240px', flex: 1, display: 'flex', flexDirection: 'column' as const, transition: 'margin-left 0.2s ease', minWidth: 0 }}>
        ...
        <div style={{ height: '56px', backgroundColor: 'var(--bg-primary)', borderBottom: '1px solid var(--sidebar-border)', display: 'flex', alignItems: 'center', padding: '0 20px', gap: '12px', position: 'sticky' as const, top: 0, zIndex: 40 }}>
          <div style={{ flex: 1, maxWidth: '380px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px', padding: '7px 12px' }}>
              ...
              <input value={searchVal} onChange={e => setSearchVal(e.target.value)} placeholder="Search BEI Intelligence..." style={{ background: 'none', border: 'none', outline: 'none', color: 'var(--text-muted)', fontSize: '17px', flex: 1, minWidth: 0 }} />
              <div style={{ display: 'flex', alignItems: 'center', gap: '2px', flexShrink: 0 }}>
                <kbd style={{ fontSize: '17px', color: 'var(--text-faint)', backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '3px', padding: '1px 4px' }}>⌘</kbd>
                <kbd style={{ fontSize: '17px', color: 'var(--text-faint)', backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '3px', padding: '1px 4px' }}>K</kbd>
              </div>
            </div>
          </div>
          <div style={{ flex: 1 }} />
          <button onClick={() => window.dispatchEvent(new CustomEvent('open-ask-bei'))} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '8px 16px', backgroundColor: 'rgba(200,162,74,0.1)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '8px', color: gold, fontSize: '17px', fontWeight: '600', cursor: 'pointer' }}>
            <span>✦</span> Ask BEI
          </button>
          <button onClick={toggleTheme} title={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'} style={{ width: '36px', height: '36px', borderRadius: '8px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '17px' }}>
            {theme === 'dark' ? '☀' : '◑'}
          </button>
          ...
          <a href="/book" style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text-muted)', fontSize: '17px', textDecoration: 'none', whiteSpace: 'nowrap' as const }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 10px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px' }}>
            <div style={{ width: '26px', height: '26px', borderRadius: '6px', backgroundColor: 'rgba(200,162,74,0.12)', border: '1px solid rgba(200,162,74,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '17px', fontWeight: '700', color: gold }}>{userInitial}</div>
            <div><div style={{ fontSize: '17px', color: '#ccc', fontWeight: '600', lineHeight: '1' }}>{userName}</div><div style={{ fontSize: '17px', color: 'var(--text-secondary)', marginTop: '2px' }}>Executive</div></div>
            <button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/login' }} style={{ background: 'none', border: 'none', color: 'var(--text-faint)', cursor: 'pointer', fontSize: '17px', padding: '2px 4px' }}>Sign out</button>
          </div>
        </div>
        ...
        <div style={{ height: '30px', backgroundColor: 'var(--bg-sidebar)', borderTop: '1px solid var(--sidebar-border)', display: 'flex', alignItems: 'center', padding: '0 20px', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            ...
            <span style={{ fontSize: '17px', color: '#4aaa4a', letterSpacing: '0.08em' }}>BEI Intelligence Active</span>
          </div>
          <span style={{ fontSize: '17px', color: '#222' }}>All systems operational</span>
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: '17px', color: '#222' }}>Last sync: Just now ✓</span>
        </div>
      </div>
    </div>
    </>
