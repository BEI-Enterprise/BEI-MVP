...
export default function DashboardPage() {
  ...
  if (loading) return (
    <main style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div>
        <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.3em', marginBottom: '12px', textAlign: 'center' as const }}>BEI INTELLIGENCE</div>
        <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.1em' }}>Loading executive intelligence...</div>
      </div>
    </main>
  )
  ...
  if (!user) return (
    <main style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' as const }}>
        <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.3em', marginBottom: '16px' }}>BEI INTELLIGENCE</div>
        <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '20px' }}>Sign in to access your Executive Command Centre.</div>
        <a href='/login' style={{ padding: '12px 28px', backgroundColor: gold, color: '#050505', fontWeight: '700', borderRadius: '6px', textDecoration: 'none', fontSize: '16px' }}>Sign In →</a>
      </div>
    </main>
  )
  ...
  return (
    <DashboardShell activeId="dashboard">
      ...
      <div style={{ marginBottom: '20px', paddingBottom: '20px', borderBottom: '1px solid ' + border }}>
        <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.25em', marginBottom: '6px', fontWeight: '600' }}>EXECUTIVE COMMAND CENTRE</div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h1 style={{ fontSize: '32px', fontWeight: '800', letterSpacing: '-0.02em', margin: '0 0 4px 0', color: 'var(--text-primary)' }}>Welcome back, {userName}.</h1>
            <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginTop: '2px' }}>{businessName} · {tier} Plan · {industryLabel}</div>
            <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginTop: '3px', letterSpacing: '0.04em' }}>Your real-time executive intelligence overview.</div>
          </div>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <a href="/book" style={{ padding: '9px 18px', border: '1px solid var(--border)', borderRadius: '6px', color: '#bbb', fontSize: '16px', textDecoration: 'none' }}>Book Strategy Session</a>
            <a href="/report" style={{ padding: '9px 18px', backgroundColor: gold, borderRadius: '6px', color: '#050505', fontSize: '16px', fontWeight: '700', textDecoration: 'none' }}>View Full Report →</a>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '10px', marginBottom: '20px' }}>
        {[
          ...
          <div key={i} style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px', padding: '14px 16px' }}>
            <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.12em', marginBottom: '8px', fontWeight: '600' }}>{k.label}</div>
            <div style={{ fontSize: '19px', fontWeight: '800', color: k.color, lineHeight: 1, marginBottom: '4px' }}>{k.value}</div>
            <div style={{ fontSize: '16px', color: k.trendColor, fontWeight: '600', marginBottom: '2px' }}>{k.trend}</div>
            <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginBottom: '8px' }}>{k.prevVal}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 300px', gap: '16px', alignItems: 'start' }}>
          ...
          {primary ? (
            <div style={{ backgroundColor: 'var(--bg-constraint-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '10px', padding: '28px', position: 'relative' as const, overflow: 'hidden' as const }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 180px 200px 200px', gap: '24px', marginBottom: '24px' }}>
                ...
                <div>
                  <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.25em', fontWeight: '600', marginBottom: '10px' }}>PRIMARY CONSTRAINT</div>
                  <h2 style={{ fontSize: '26px', fontWeight: '900', color: gold, marginBottom: '10px', letterSpacing: '-0.02em', lineHeight: 1.1 }}>{primary.name}™</h2>
                  <p style={{ fontSize: '16px', color: 'var(--text-secondary)', lineHeight: '1.7', marginBottom: '16px' }}>{primary.hypothesis || 'Leadership capacity and decision velocity are limiting execution speed and business growth.'}</p>
                  <div style={{ marginBottom: '14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.15em' }}>VERIFICATION SCORE</div>
                      <div style={{ fontSize: '16px', fontWeight: '800', color: verificationScore >= 70 ? '#4aaa4a' : gold }}>{verificationScore}/100</div>
                    </div>
                  ...
                  <div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '3px' }}>CONFIDENCE LEVEL</div>
                    <div style={{ fontSize: '20px', fontWeight: '800', color: confColor }}>{confidence.toUpperCase()}</div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>85% confidence</div>
                  </div>
                </div>

                <div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '8px' }}>ANNUAL OPPORTUNITY</div>
                  <div style={{ fontSize: '20px', fontWeight: '900', color: gold, marginBottom: '2px', lineHeight: 1.1 }}>{oppLow > 0 ? fmt(oppLow) : '—'}</div>
                  <div style={{ fontSize: '16px', fontWeight: '700', color: gold + 'bb', marginBottom: '16px' }}>{oppHigh > 0 ? '– ' + fmt(oppHigh) : ''}</div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '10px' }}>AFFECTED AREAS</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '6px' }}>
                    {affectedAreas.slice(0, 4).map((area: string, i: number) => (
                      <div key={i} style={{ padding: '4px 10px', backgroundColor: 'rgba(200,162,74,0.1)', border: '1px solid rgba(200,162,74,0.25)', borderRadius: '4px', fontSize: '16px', color: gold, fontWeight: '600' }}>{area}</div>
                    ))}
                  </div>
                </div>

                <div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '8px' }}>WHY IT MATTERS</div>
                  <p style={{ fontSize: '16px', color: 'var(--text-secondary)', lineHeight: '1.7', marginBottom: '14px' }}>
                  ...
                  <div style={{ backgroundColor: 'rgba(200,162,74,0.07)', border: '1px solid rgba(200,162,74,0.2)', borderRadius: '6px', padding: '10px 12px', marginBottom: '10px' }}>
                    <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.15em', marginBottom: '5px', fontWeight: '600' }}>RECOMMENDED NEXT ACTION</div>
                    <div style={{ fontSize: '16px', color: 'var(--text-primary)', fontWeight: '600', lineHeight: '1.4' }}>Deploy Tier 1 {primary.name} Framework →</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.15em', marginBottom: '4px' }}>EXPECTED OUTCOME</div>
                ...
                <div style={{ display: 'flex', gap: '20px' }}>
                  {primary.evidence?.slice(0, 2).map((e: string, idx: number) => (
                    <div key={idx} style={{ display: 'flex', gap: '6px', alignItems: 'flex-start', maxWidth: '280px' }}>
                      <span style={{ color: gold, flexShrink: 0, fontSize: '16px', marginTop: '2px' }}>◈</span>
                      <span style={{ fontSize: '16px', color: 'var(--text-muted)', lineHeight: '1.5' }}>{e}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
                  <a href="/constraints" style={{ padding: '9px 20px', backgroundColor: gold, color: '#050505', borderRadius: '6px', fontSize: '16px', fontWeight: '700', textDecoration: 'none', whiteSpace: 'nowrap' as const }}>View Constraint Intelligence →</a>
                  <a href="/deployments" style={{ padding: '9px 20px', border: '1px solid var(--border)', color: 'var(--text-muted)', borderRadius: '6px', fontSize: '16px', textDecoration: 'none', whiteSpace: 'nowrap' as const }}>Deployment Packages</a>
                </div>
              </div>
            </div>
          ) : (
            <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '40px', textAlign: 'center' as const }}>
              <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '12px' }}>PRIMARY CONSTRAINT</div>
              ...
              <a href="/book" style={{ padding: '10px 24px', backgroundColor: gold, color: '#050505', borderRadius: '6px', fontWeight: '700', textDecoration: 'none', fontSize: '16px' }}>Generate Business MRI →</a>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div style={{ fontSize: '16px', color: '#dddddd', letterSpacing: '0.2em', fontWeight: '600' }}>EXECUTIVE ACTION CENTRE</div>
                <a href="/deployments" style={{ fontSize: '16px', color: gold, textDecoration: 'none' }}>View all actions →</a>
              </div>
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', backgroundColor: a.bg, border: '1px solid ' + a.color + '33', borderRadius: '6px', marginBottom: i < 3 ? '8px' : '0' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                      <div style={{ fontSize: '16px', color: a.color, fontWeight: '700', letterSpacing: '0.1em' }}>{a.priority}</div>
                      <div style={{ fontSize: '16px', color: a.color, fontWeight: '600' }}>{a.value}</div>
                    </div>
                    <div style={{ fontSize: '16px', color: 'var(--text-primary)', fontWeight: '600', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{a.title}</div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginTop: '1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{a.desc}</div>
                  </div>
            ...
            <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div style={{ fontSize: '16px', color: '#dddddd', letterSpacing: '0.2em', fontWeight: '600' }}>DECISION INTELLIGENCE FEED</div>
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <div style={{ width: '5px', height: '5px', borderRadius: '50%', backgroundColor: '#4aaa4a', boxShadow: '0 0 4px rgba(74,170,74,0.6)' }} />
                  <span style={{ fontSize: '16px', color: '#4aaa4a', letterSpacing: '0.1em' }}>LIVE</span>
                </div>
              </div>
              {feed.map((f, i) => (
                <div key={i} style={{ display: 'flex', gap: '10px', padding: '10px 0', borderBottom: i < feed.length - 1 ? '1px solid ' + border : 'none' }}>
                  <div style={{ flexShrink: 0, marginTop: '2px' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: i === 0 ? 'rgba(200,162,74,0.15)' : i === 1 ? 'rgba(74,138,176,0.15)' : 'rgba(74,170,74,0.15)', border: '1px solid ' + (i === 0 ? gold + '44' : i === 1 ? '#4a8ab044' : '#4aaa4a44'), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div style={{ fontSize: '16px', color: i === 0 ? gold : i === 1 ? '#4a8ab0' : '#4aaa4a' }}>{i === 0 ? '★' : i === 1 ? '↗' : '◎'}</div>
                    </div>
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: '16px', color: 'var(--text-secondary)', lineHeight: '1.6', margin: '0 0 3px 0' }}>{f.text}</p>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{f.time}</div>
                  </div>
                </div>
              ))}
              <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid ' + border, textAlign: 'center' as const }}>
                <a href="/insights" style={{ fontSize: '16px', color: gold, textDecoration: 'none' }}>View all insights →</a>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          ...
          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ fontSize: '16px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '700' }}>BUSINESS HEALTH RADAR</div>
              <button onClick={() => setShowRadarModal(true)} style={{ fontSize: '16px', color: gold, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontWeight: '600' }}>View full →</button>
            </div>
            {pillarList.length > 0 ? (
              <>
                ...
                <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    ...
                    <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Your score</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                    <svg width="18" height="6"><line x1="0" y1="3" x2="18" y2="3" stroke={gold} strokeWidth="1" strokeDasharray="3 3" strokeOpacity="0.5"/></svg>
                    <span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Industry benchmark</span>
                  </div>
                </div>
                <div style={{ textAlign: 'center' as const, marginBottom: '12px' }}>
                  <div style={{ fontSize: '32px', fontWeight: '900', color: healthColor, lineHeight: 1 }}>{healthScore}</div>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginTop: '3px', letterSpacing: '0.1em' }}>HEALTH SCORE</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '6px' }}>
                  {pillarList.map((p: any, i: number) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: p.color, flexShrink: 0 }} />
                      <div style={{ flex: 1, fontSize: '16px', color: 'var(--text-muted)' }}>{p.name}</div>
                      <div style={{ fontSize: '16px', fontWeight: '700', color: p.color }}>{p.score}/20</div>
              ...
              <div style={{ textAlign: 'center' as const, padding: '20px 0', color: 'var(--text-muted)', fontSize: '16px' }}>Generate MRI to see health radar</div>
            )}
          </div>

          <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <div style={{ fontSize: '16px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>INTELLIGENCE ALERTS</div>
              <a href="/constraints" style={{ fontSize: '16px', color: gold, textDecoration: 'none' }}>View all →</a>
            </div>
                ...
                <div key={i} style={{ display: 'flex', gap: '10px', padding: '10px 0', borderBottom: i < alerts.length-1 ? '1px solid ' + border : 'none' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                      <div style={{ fontSize: '16px', color: c, fontWeight: '700', letterSpacing: '0.1em' }}>{levelLabel}</div>
                      <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{a.time}</div>
                    </div>
                    <div style={{ fontSize: '16px', color: 'var(--text-primary)', fontWeight: '600' }}>{a.title}</div>
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)', marginTop: '2px' }}>{a.desc}</div>
                  </div>
                </div>
              )
            })}
          ...
          {oppHigh > 0 && (
            <div style={{ backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                <div style={{ fontSize: '16px', color: '#dddddd', letterSpacing: '0.15em', fontWeight: '600' }}>OPPORTUNITY SNAPSHOT</div>
                <a href="/opportunities" style={{ fontSize: '16px', color: gold, textDecoration: 'none' }}>View pipeline →</a>
              </div>
                ...
                <div style={{ flex: 1 }}>
                  {oppBreakdown.map((o, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: i < oppBreakdown.length - 1 ? '5px' : '0' }}>
                      ...
                      <div style={{ flex: 1, fontSize: '16px', color: 'var(--text-muted)' }}>{o.label}</div>
                      <div style={{ fontSize: '16px', color: o.color, fontWeight: '600' }}>{o.val}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ borderTop: '1px solid ' + border, paddingTop: '10px', display: 'flex', justifyContent: 'space-between' }}>
                <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Avg. Confidence: <span style={{ color: confColor, fontWeight: '600' }}>78%</span></div>
                <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{oppBreakdown.length} opportunities</div>
              </div>
            </div>
          )}
        ...
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.9)', zIndex: 300, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '40px', overflowY: 'auto' as const }} onClick={() => setShowRadarModal(false)}>
          <div style={{ backgroundColor: 'var(--bg-card)', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '14px', padding: '32px', width: '740px', maxWidth: '95vw', marginBottom: '40px' }} onClick={(e: any) => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
              <div>
                <div style={{ fontSize: '16px', color: gold, letterSpacing: '0.2em', marginBottom: '6px', fontWeight: '600' }}>BUSINESS HEALTH RADAR</div>
                <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--text-primary)', marginBottom: '4px' }}>Full Business Health Analysis</div>
                <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{businessName} · Health Score: {healthScore}/100 · {pillarList.length} pillars</div>
              </div>
              <button onClick={() => setShowRadarModal(false)} style={{ background: 'none', border: '1px solid #2a2a2a', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '20px', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '20px' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', marginTop: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '3px', backgroundColor: gold }}/><span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Your score</span></div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}><div style={{ width: '10px', height: '2px', backgroundColor: gold, opacity: 0.4 }}/><span style={{ fontSize: '16px', color: 'var(--text-muted)' }}>Industry benchmark</span></div>
                </div>
              </div>
                  ...
                  <div key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '16px', color: 'var(--text-secondary)', fontWeight: '700' }}>{p.name}</span>
                      </div>
                      <span style={{ fontSize: '16px', color: p.color, fontWeight: '900' }}>{p.score}/20</span>
                    </div>
                    ...
                    <div style={{ fontSize: '16px', color: 'var(--text-muted)' }}>{p.label || (p.score >= 14 ? 'Strong' : p.score >= 9 ? 'Moderate' : 'Needs Attention')}</div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '10px' }}>
              {pillarList.map((p: any, i: number) => (
                <div key={i} style={{ padding: '14px', backgroundColor: 'var(--bg-sidebar)', borderRadius: '8px', border: '1px solid '+p.color+'33', textAlign: 'center' as const }}>
                  <div style={{ fontSize: '16px', color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: '6px', fontWeight: '600' }}>{p.name.toUpperCase()}</div>
                  <div style={{ fontSize: '20px', fontWeight: '900', color: p.color }}>{p.score}/20</div>
                  <div style={{ fontSize: '16px', color: p.score >= 14 ? '#4aaa4a' : p.score >= 9 ? gold : '#e8923a', marginTop: '3px', fontWeight: '600' }}>{p.label || (p.score >= 14 ? 'Strong' : p.score >= 9 ? 'Moderate' : 'Needs Attention')}</div>
                </div>
              ))}
            </div>
          </div>
