...
export default function DashboardPage() {
  ...
  if (loading) return (
    <main style={{ backgroundColor: '#0c0c0c', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ fontSize: '13px', color: '#999', letterSpacing: '0.1em' }}>Loading your intelligence...</div>
    </main>
  )
      ...
      {showWelcome && (
        <div style={{ position: 'fixed' as const, inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)' }}>
          <div style={{ maxWidth: '560px', padding: '56px', backgroundColor: '#141414', border: '1px solid rgba(200,162,74,0.3)', borderRadius: '16px', textAlign: 'center' as const, position: 'relative' as const }}>
            ...
            <div style={{ fontSize: '15px', color: '#999', lineHeight: '1.75', marginBottom: '32px' }}>
          ...
          <div style={{ position: 'relative' as const, zIndex: 1, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px' }}>
            ...
            <div style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
              <div style={{ textAlign: 'center' as const }}>
                ...
                <div style={{ fontSize: '10px', color: '#999', marginTop: '3px' }}>HEALTH</div>
              </div>
              ...
              <div style={{ textAlign: 'center' as const }}>
                ...
                <div style={{ fontSize: '10px', color: '#999', marginTop: '3px' }}>VERIFICATION</div>
              </div>
              <div style={{ width: '1px', height: '32px', backgroundColor: '#1a1a1a' }} />
              <div style={{ textAlign: 'center' as const }}>
                ...
                <div style={{ fontSize: '10px', color: '#999', marginTop: '3px' }}>OPPORTUNITY</div>
              </div>
              <div style={{ width: '1px', height: '32px', backgroundColor: '#1a1a1a' }} />
              <div style={{ display: 'flex', gap: '10px' }}>
                <a href='/account' style={{ padding: '8px 14px', border: '1px solid #1a1a1a', borderRadius: '4px', fontSize: '12px', color: '#999', textDecoration: 'none', backdropFilter: 'blur(4px)' }}>Account</a>
                <button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/' }} style={{ padding: '8px 14px', border: '1px solid #1a1a1a', borderRadius: '4px', fontSize: '12px', color: '#999', backgroundColor: 'transparent', cursor: 'pointer' }}>Sign out</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', minHeight: 'calc(100vh - 68px)' }}>
        ...
        <div style={{ borderRight: '1px solid #161616', backgroundColor: '#0e0e0e', padding: '28px 0', display: 'flex', flexDirection: 'column' as const, position: 'relative' as const, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '0 20px 24px', borderBottom: '1px solid #222' }}>
            <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', textTransform: 'uppercase' as const, marginBottom: '12px', fontWeight: '600' }}>Businesses</div>
            {businesses.length === 0 ? (
              <div style={{ fontSize: '13px', color: '#999' }}>No businesses found</div>
            ) : businesses.map(b => (
              <button key={b.id} onClick={() => setSelected(b)} style={{ width: '100%', padding: '10px 12px', marginBottom: '4px', backgroundColor: selected?.id === b.id ? 'rgba(200,162,74,0.08)' : 'transparent', border: selected?.id === b.id ? '1px solid rgba(200,162,74,0.2)' : '1px solid transparent', borderRadius: '6px', textAlign: 'left' as const, cursor: 'pointer', display: 'block' }}>
                ...
                <div style={{ fontSize: '11px', color: '#888' }}>{b.subscription_tier || 'analysis'} plan</div>
              </button>
            ))}
          </div>
          ...
          <div style={{ marginTop: 'auto', padding: '20px', borderTop: '1px solid #111' }}>
            {[['Account', '/account'], ['Book Session', '/book']].map(([label, href]) => (
              <a key={href} href={href} style={{ display: 'block', padding: '8px 0', fontSize: '13px', color: '#999', textDecoration: 'none' }}>{label}</a>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '32px 40px', overflowY: 'auto' as const }}>
          ...
          <div style={{ marginBottom: '32px', paddingBottom: '24px', borderBottom: '1px solid #222', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div style={{ fontSize: '13px', color: '#999', marginTop: '6px' }}>Last updated: {selected?.updated_at ? new Date(selected.updated_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : 'Never'}</div>
            </div>
            <div style={{ display: 'flex', gap: '10px' }}>
          ...
          {activeTab === 'overview' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '20px', marginBottom: '24px' }}>
                ...
                >
                  <div style={{ fontSize: '9px', color: '#999', letterSpacing: '0.2em', marginBottom: '14px' }}>OVERALL HEALTH</div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr 1fr', gap: '10px' }}>
                  {[
                    ...
                    >
                      <div style={{ fontSize: '9px', color: '#888', letterSpacing: '0.15em', marginBottom: '8px' }}>{s.label.toUpperCase()}</div>
              ...
              {primary && (
                <div style={{ padding: '20px 24px', backgroundColor: '#141414', border: '1px solid #2a2a2a', borderRadius: '8px', marginBottom: '20px' }}>
                  <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', marginBottom: '14px', fontWeight: '600' }}>CONSTRAINT DETECTION ENGINE — LIVE</div>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                      <div style={{ width: '160px', fontSize: '11px', color: '#999', flexShrink: 0, whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name}</div>
              ...
              {health.pillars && Object.keys(health.pillars).length > 0 && (
                <div style={{ padding: '24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', marginBottom: '20px' }}>
                  <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>PILLAR SCORES</div>
                    ...
                    {Object.entries(health.pillars).map(([name, data]: [string, any]) => {
                      ...
                      return (
                        <div key={name} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ width: '80px', fontSize: '12px', color: '#aaa', textTransform: 'capitalize' as const }}>{name}</div>
                  ...
                  <div style={{ padding: '24px', backgroundColor: '#141414', border: '1px solid #1a1a1a', borderRadius: '10px', marginBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                      ...
                      <div style={{ textAlign: 'right' as const, flexShrink: 0, marginLeft: '20px' }}>
                        ...
                        <div style={{ fontSize: '10px', color: '#999', marginTop: '2px' }}>connected</div>
                      </div>
                    </div>
                      ...
                      {connectors.map((c, i) => (
                        <div key={c.id} style={{ display: 'flex', gap: '10px', alignItems: 'center', padding: '10px 12px', backgroundColor: '#0a0a0a', border: '1px solid #2a2a2a', borderRadius: '6px' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontSize: '12px', fontWeight: '600', color: '#999', whiteSpace: 'nowrap' as const, overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.label}</div>
                            <div style={{ fontSize: '10px', color: '#888', marginTop: '1px' }}>{c.desc}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <span style={{ fontSize: '12px', color: '#999', marginLeft: '16px' }}>Takes 2–5 minutes per connector</span>
                  </div>
                )
              })()}
              ...
              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                  <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', fontWeight: '600' }}>
                ...
                <div style={{ padding: '28px', backgroundColor: '#080f04', border: '1px solid #2a3a1a', borderRadius: '10px', marginBottom: '20px' }}>
                  ...
                  {primary.evidence && primary.evidence.slice(0,2).map((e: string, i: number) => (
                    <div key={i} style={{ display: 'flex', gap: '10px', marginBottom: '8px' }}>
                      ...
                      <span style={{ fontSize: '13px', color: '#999', lineHeight: '1.6' }}>{e}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activeTab === 'reports' && (
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>ANALYSIS REPORTS</div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {businesses.length > 1 && (
                      <div style={{ marginBottom: '8px' }}>
                        <div style={{ fontSize: '11px', color: '#999', marginBottom: '8px', letterSpacing: '0.1em' }}>SELECT BUSINESS TO ANALYSE</div>
              ...
              <div style={{ padding: '28px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <div style={{ textAlign: 'right' as const }}>
                      <div style={{ fontSize: '11px', color: '#999', marginBottom: '4px' }}>HEALTH SCORE</div>
                      <div style={{ fontSize: '28px', fontWeight: '800', color: healthColor }}>{health.overall || '—'}</div>
                    </div>
                ...
                {primary && (
                  <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #111', display: 'flex', gap: '32px' }}>
                    <div>
                      <div style={{ fontSize: '10px', color: '#999', marginBottom: '4px' }}>PRIMARY CONSTRAINT</div>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#f0f0f0' }}>{primary.name}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: '10px', color: '#999', marginBottom: '4px' }}>VERIFICATION</div>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#4aaa4a' }}>{primary.verification_score}/100</div>
                    </div>
                    <div>
                      <div style={{ fontSize: '10px', color: '#999', marginBottom: '4px' }}>SEVERITY</div>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#cc4444' }}>{(primary.severity || 'medium').toUpperCase()}</div>
                    </div>
                  </div>
                )}
              </div>
              <div style={{ padding: '20px 24px', backgroundColor: '#141414', border: '1px solid #2a2a2a', borderRadius: '8px', fontSize: '13px', color: '#999', lineHeight: '1.7' }}>
          ...
          {activeTab === 'revenue' && (
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>BUSINESS HEALTH</div>
              ...
              {result ? (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: '20px', marginBottom: '24px' }}>
                    <div
                      ...
                      onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}
                    >
                      <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.15em', marginBottom: '12px' }}>OVERALL HEALTH</div>
                      ...
                      {health.pillars && Object.entries(health.pillars).map(([name, data]: [string, any]) => {
                        ...
                        return (
                          <div key={name}>
                            ...
                            {data.band && <div style={{ fontSize: '10px', color: '#888', marginTop: '3px', textTransform: 'capitalize' as const }}>{data.band}</div>}
                          </div>
                        )
                      })}
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '20px' }}>
                    {[
                      ...
                      <div
                        ...
                        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.transform = 'translateY(0)'; el.style.boxShadow = 'none'; el.style.backgroundColor = '#0e0e0e' }}
                      >
                        <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.15em', marginBottom: '6px' }}>{s.label.toUpperCase()}</div>
                  ...
                  {result.total_opportunity && (
                    <div style={{ padding: '24px', backgroundColor: card, border: '1px solid rgba(200,162,74,0.2)', borderRadius: '10px', marginBottom: '16px' }}>
                      <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>OPPORTUNITY IMPACT ON HEALTH</div>
                      <div style={{ display: 'flex', gap: '48px', alignItems: 'flex-end' }}>
                        <div>
                          <div style={{ fontSize: '10px', color: '#999', marginBottom: '6px' }}>CONSERVATIVE UPLIFT</div>
                          ...
                          <div style={{ fontSize: '11px', color: '#999', marginTop: '4px' }}>Annual revenue gain</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '10px', color: '#999', marginBottom: '6px' }}>OPTIMISTIC UPLIFT</div>
                        ...
                        <div style={{ flex: 1, paddingLeft: '20px', borderLeft: '1px solid #161616' }}>
                          <div style={{ fontSize: '10px', color: '#999', marginBottom: '8px' }}>PRIMARY CONSTRAINT RESOLUTION</div>
                  ...
                  <div style={{ padding: '16px 20px', backgroundColor: '#141414', border: '1px solid #2a2a2a', borderRadius: '8px', fontSize: '13px', color: '#999', lineHeight: '1.7' }}>
                ...
                <div style={{ padding: '48px', textAlign: 'center' as const, backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px' }}>
                  <div style={{ fontSize: '13px', color: '#999', marginBottom: '20px' }}>Complete your MRI to see your full business health breakdown.</div>
          ...
          {activeTab === 'issues' && (
            <div>
              <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>ISSUES & CONSTRAINTS</div>
              <div style={{ fontSize: '13px', color: '#aaa', marginBottom: '24px', lineHeight: '1.7' }}>
              ...
              {primary ? (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '20px' }}>
                    {[
                      ...
                      <div key={s.label} style={{ padding: '14px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px' }}>
                        <div style={{ fontSize: '10px', color: '#999', marginBottom: '6px', letterSpacing: '0.1em' }}>{s.label.toUpperCase()}</div>
                  ...
                  <div style={{ padding: '24px', backgroundColor: '#1a0a0a', border: '1px solid #3a1a1a', borderLeft: '3px solid #cc4444', borderRadius: '8px', marginBottom: '12px' }}>
                    ...
                    {primary.evidence && primary.evidence.length > 0 && (
                      <div style={{ marginBottom: '14px' }}>
                        <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.15em', marginBottom: '8px' }}>EVIDENCE</div>
                        {primary.evidence.slice(0,3).map((e: string, i: number) => (
                          <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '6px', alignItems: 'flex-start' }}>
                            ...
                            <span style={{ fontSize: '12px', color: '#999', lineHeight: '1.6' }}>{e}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  {secondary.filter((c: any) => c.severity === 'high').map((c: any) => (
                    <div key={c.key} style={{ padding: '20px 24px', backgroundColor: '#0f0a04', border: '1px solid #2a2000', borderLeft: '3px solid #C8A24A', borderRadius: '8px', marginBottom: '12px' }}>
                      ...
                      <div style={{ fontSize: '13px', color: '#999', lineHeight: '1.6' }}>{c.hypothesis}</div>
                    </div>
                  ))}
                  {secondary.filter((c: any) => c.severity !== 'high').map((c: any) => (
                ...
                <div style={{ padding: '40px', textAlign: 'center' as const, backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px', color: '#999', fontSize: '13px' }}>
        ...
        {/* CONNECTORS TAB */}
          {activeTab === 'connectors' && (
            <div>
              <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>DATA CONNECTORS</div>
              ...
              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', marginBottom: '14px', fontWeight: '600' }}>CRM & SALES</div>
              ...
              {/* Finance connectors */}
              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', marginBottom: '14px', fontWeight: '600' }}>FINANCE & ACCOUNTING</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                  {[
              ...
              {/* HR & Operations */}
              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.2em', marginBottom: '14px', fontWeight: '600' }}>HR, OPERATIONS & ANALYTICS</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
                  {[
          ...
          {activeTab === 'deployment' && (
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#999', letterSpacing: '0.2em', marginBottom: '8px', fontWeight: '600' }}>OUTCOME & DEPLOYMENT CENTRE</div>
              <div style={{ fontSize: '13px', color: '#aaa', marginBottom: '28px', lineHeight: '1.7' }}>
              ...
              <div style={{ marginBottom: '40px' }}>
                  ...
                  {result?.deployment_packages?.tier1_automatic?.length > 0 ? (
                    result.deployment_packages.tier1_automatic.map((pkg: any) => (
                      <div key={pkg.deployment_id} style={{ padding: '18px 20px', backgroundColor: '#080f04', border: '1px solid #1a3a1a', borderLeft: '3px solid #4aaa4a', borderRadius: '8px', marginBottom: '10px' }}>
                        ...
                        <div style={{ fontSize: '12px', color: '#999', marginBottom: '8px' }}>Constraint: {pkg.constraint_name}</div>
                    ...
                    <div style={{ padding: '16px 20px', backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '8px', fontSize: '13px', color: '#aaa' }}>
                  ...
                  {result?.deployment_packages?.tier2_approval?.length > 0 ? (
                    result.deployment_packages.tier2_approval.map((pkg: any) => (
                      <div key={pkg.deployment_id} style={{ padding: '18px 20px', backgroundColor: '#0f0a04', border: '1px solid #3a2a04', borderLeft: '3px solid #C8A24A', borderRadius: '8px', marginBottom: '10px' }}>
                        ...
                        {pkg.measurement_plan && <div style={{ fontSize: '11px', color: '#888' }}>Measurement: {pkg.measurement_plan}</div>}
                      </div>
                    ))
                  ) : (
                    <div style={{ padding: '16px 20px', backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '8px', fontSize: '13px', color: '#aaa' }}>
                      ...
                      <div key={pkg.deployment_id} style={{ padding: '18px 20px', backgroundColor: card, border: '1px solid #1a2a3a', borderLeft: '3px solid #4a8ab0', borderRadius: '8px', marginBottom: '10px' }}>
                        ...
                        {pkg.implementation_steps && pkg.implementation_steps.length > 0 && (
                          <div style={{ marginTop: '8px' }}>
                            <div style={{ fontSize: '10px', color: '#aaa', marginBottom: '6px', letterSpacing: '0.1em' }}>IMPLEMENTATION STEPS</div>
                            {pkg.implementation_steps.slice(0,3).map((step: string, i: number) => (
                              <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '4px', fontSize: '12px', color: '#999' }}>
                ...
                <div style={{ padding: '20px 24px', backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '8px' }}>
                  <div style={{ fontSize: '10px', color: '#aaa', letterSpacing: '0.2em', marginBottom: '12px', fontWeight: '600' }}>DEPLOYMENT HISTORY</div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '20px' }}>
                  {[
                    ...
                    <div key={m.label} style={{ padding: '16px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px' }}>
                      <div style={{ fontSize: '10px', color: '#888', marginBottom: '6px', letterSpacing: '0.1em' }}>{m.label.toUpperCase()}</div>
                ...
                <div style={{ padding: '20px 24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px', marginBottom: '16px' }}>
                  <div style={{ fontSize: '10px', color: '#aaa', letterSpacing: '0.2em', marginBottom: '12px', fontWeight: '600' }}>OUTCOME TRACKING</div>
              ...
              {!result && (
                <div style={{ padding: '40px', textAlign: 'center' as const, backgroundColor: 'rgba(200,162,74,0.04)', border: '1px solid rgba(200,162,74,0.15)', borderRadius: '10px', marginTop: '24px' }}>
                  ...
                  <div style={{ fontSize: '13px', color: '#888', marginBottom: '24px', maxWidth: '480px', margin: '0 auto 24px', lineHeight: '1.7' }}>
              ...
              {result && result.deployment_packages ? (
                <>
                  ...
                  {result.total_opportunity && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '28px' }}>
                      <div style={{ padding: '20px', backgroundColor: card, border: '1px solid rgba(200,162,74,0.2)', borderRadius: '8px' }}>
                        <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.15em', marginBottom: '8px' }}>TOTAL OPPORTUNITY</div>
                        ...
                        <div style={{ fontSize: '11px', color: '#888', marginTop: '4px' }}>Annual value available</div>
                      </div>
                      <div style={{ padding: '20px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px' }}>
                        <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.15em', marginBottom: '8px' }}>DEPLOYMENTS READY</div>
                        ...
                        <div style={{ fontSize: '11px', color: '#888', marginTop: '4px' }}>Across all tiers</div>
                      </div>
                      <div style={{ padding: '20px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '8px' }}>
                        <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.15em', marginBottom: '8px' }}>PRIMARY CONSTRAINT</div>
                        <div style={{ fontSize: '14px', fontWeight: '700', color: '#cc4444', lineHeight: '1.3' }}>{primary?.name || '—'}</div>
                        <div style={{ fontSize: '11px', color: '#888', marginTop: '4px' }}>Root cause confirmed</div>
                      </div>
                    </div>
                  )}
                    ...
                    <div style={{ marginBottom: '24px' }}>
                      ...
                      {(result.deployment_packages.tier1_automatic || []).map((pkg: any, i: number) => (
                        <div key={i} style={{ padding: '20px 24px', backgroundColor: '#080f04', border: '1px solid #1a3a1a', borderLeft: '3px solid #4aaa4a', borderRadius: '8px', marginBottom: '10px' }}>
                          ...
                          <div style={{ fontSize: '13px', color: '#999', lineHeight: '1.6', marginBottom: '10px' }}>{pkg.description || pkg.rationale || ''}</div>
                    ...
                    <div style={{ marginBottom: '24px' }}>
                      ...
                      {(result.deployment_packages.tier3_recommendation || []).map((pkg: any, i: number) => (
                        <div key={i} style={{ padding: '20px 24px', backgroundColor: card, border: '1px solid #1a2a3a', borderLeft: '3px solid #4a8ab0', borderRadius: '8px', marginBottom: '10px' }}>
                          ...
                          <div style={{ fontSize: '13px', color: '#999', lineHeight: '1.6' }}>{pkg.description || pkg.rationale || ''}</div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  <div style={{ padding: '24px', backgroundColor: card, border: '1px solid ' + border, borderRadius: '10px' }}>
                    <div style={{ fontSize: '10px', color: '#999', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>OUTCOME TRACKING</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '16px' }}>
                      {[
                        ...
                        <div key={m.label} style={{ padding: '16px', backgroundColor: '#141414', borderRadius: '6px', border: '1px solid #2a2a2a' }}>
                          <div style={{ fontSize: '10px', color: '#888', marginBottom: '6px' }}>{m.label.toUpperCase()}</div>
          ...
          {activeTab === 'intelligence' && (
            <div>
                  ...
                  <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                    {[
                      ...
                      <div key={d.label} style={{ textAlign: 'center' as const }}>
                        ...
                        <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.1em' }}>{d.label.toUpperCase()}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ marginBottom: '28px' }}>
                <div style={{ fontSize: '11px', color: '#888', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                  {industryData.signals.map((sig: any) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: '12px', color: '#999', lineHeight: '1.7' }}>{sig.signal}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Industry benchmarks */}
              <div style={{ marginBottom: '28px' }}>
                <div style={{ fontSize: '11px', color: '#888', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>INDUSTRY BENCHMARKS — {industryData.label.toUpperCase()}</div>
                <div style={{ backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '10px', overflow: 'hidden' }}>
                  {industryData.benchmarks.map((b: any, i: number) => (
                    <div key={b.metric} style={{ display: 'grid', gridTemplateColumns: '1fr 180px 1fr', gap: '0', borderBottom: i < industryData.benchmarks.length - 1 ? '1px solid #111' : 'none', alignItems: 'center' }}>
                      <div style={{ padding: '14px 20px', fontSize: '13px', color: '#999' }}>{b.metric}</div>
              ...
              <div style={{ marginBottom: '28px' }}>
                <div style={{ fontSize: '11px', color: '#888', letterSpacing: '0.2em', marginBottom: '16px', fontWeight: '600' }}>MONITORING COVERAGE — 4 SIGNAL DOMAINS</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                  {[
                    ...
                    >
                      <div style={{ fontSize: '10px', color: '#888', letterSpacing: '0.15em', marginBottom: '12px' }}>{g.label.toUpperCase()}</div>
              ...
              <div style={{ padding: '16px 24px', backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#888' }}>Monitoring {industryData.label} · Human-verified intelligence · No alert sent without BEI team validation</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' as const }}>
